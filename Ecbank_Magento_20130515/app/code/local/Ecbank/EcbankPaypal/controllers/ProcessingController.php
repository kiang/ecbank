...
class Ecbank_EcbankPaypal_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'ecbankpaypal/success';
    protected $_failureBlockType = 'ecbankpaypal/failure';
    protected $_cancelBlockType = 'ecbankpaypal/cancel';
    ...
    /**
     * when customer selects ecbankpaypal payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage::helper('ecbankpaypal')->__('轉向至PayPal。')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setPaypalQuoteId($session->getQuoteId());
                $session->setPaypalSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setPaypalRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setPaypalQuoteId($session->getQuoteId());
				//print_r($session);
				//exit;
                $session->clear();
            }
    ...
    /**
     * ecbankpaypal returns POST variables to this action
     */
    public function responseAction()
    {
        try {
            ...
            if ($request['succ'] == 1) {
                ...
                Mage::throwException('交易失敗，訂單將自動取消。');
    ...
    /**
     * ecbankpaypal return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsPaypalRealOrderId();
            $session->setQuoteId($session->getPaypalQuoteId(true));
            $session->setLastSuccessQuoteId($session->getPaypalSuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getPaypalQuoteId()) {
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('ecbankpaypal')->__('使用者已取消交易。')
        )->save();
        $session->addError(Mage::helper('ecbankpaypal')->__('使用者已取消交易。'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from ecbankpaypal/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as ecbankpaypal-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$key = Mage::getModel('Ecbank_EcbankPaypal_Model_Paypal')->getConfigData('private_key');	//在controller須先呼叫model cvs後才能使用getConfigData()
        ...
        //switch($request['authMode']) {
            //case 'A':
                if ($this->_order->canInvoice()) {
                    $invoice = $this->_order->prepareInvoice();		//產生發票
                ...
                $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('ecbankpaypal')->__('繳費完成。'));
        $this->_order->save();
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('ecbankpaypal')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('ecbankpaypal')->getPendingPaymentStatus();
    }
}