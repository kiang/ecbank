...
class Ecbank_EcbankWebatm_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'ecbankwebatm/success';
    protected $_failureBlockType = 'ecbankwebatm/failure';
    protected $_cancelBlockType = 'ecbankwebatm/cancel';
    ...
    /**
     * when customer selects ecbankwebatm payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage::helper('ecbankwebatm')->__('轉向至Ecbank中，請稍候...')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                ...
                $session->setWebatmSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setWebatmRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setWebatmQuoteId($session->getQuoteId());
				//print_r($session);
				//exit;
                $session->clear();
            }
    ...
    /**
     * ecbankwebatm returns POST variables to this action
     */
    ...
    /**
     * ecbankwebatm return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsWebatmRealOrderId();
            $session->setQuoteId($session->getWebatmQuoteId(true));
            $session->setLastSuccessQuoteId($session->getWebatmSuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getWebatmQuoteId()) {
        ...
        $session->addError(Mage::helper('ecbankwebatm')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from ecbankwebatm/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as ecbankwebatm-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction
		//驗證繳款完成通知是否由綠界傳來
		$key = Mage::getModel('Ecbank_EcbankWebatm_Model_Cc')->getConfigData('private_key');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('ecbankwebatm')->__('繳費完成。'));
        $this->_order->save();
        $this->getResponse()->setBody(
            $this->getLayout()
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('ecbankwebatm')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('ecbankwebatm')->getPendingPaymentStatus();
    }
