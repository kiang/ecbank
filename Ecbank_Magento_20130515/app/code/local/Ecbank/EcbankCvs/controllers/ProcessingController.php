...
class Ecbank_EcbankCvs_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'ecbankcvs/success';
    protected $_failureBlockType = 'ecbankcvs/failure';
    protected $_cancelBlockType = 'ecbankcvs/cancel';
    ...
    /**
     * when customer selects ecbankcvs payment method
     */
    public function redirectAction()
    {
        try {
			...
			//判斷金額是否小於30或大於20000，金額不符訂單將自動取消。
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('ecbankcvs')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setCvsQuoteId($session->getQuoteId());
                $session->setCvsSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setCvsRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setCvsQuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * ecbankcvs returns POST variables to this action
     */
    ...
    /**
     * ecbankcvs return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsCvsRealOrderId();
            $session->setQuoteId($session->getCvsQuoteId(true));
            $session->setLastSuccessQuoteId($session->getCvsSuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getCvsQuoteId()) {
        ...
        $session->addError(Mage::helper('ecbankcvs')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from ecbankcvs/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as ecbankcvs-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 			
		//驗證繳款完成通知是否由綠界傳來
		$key = Mage::getModel('Ecbank_EcbankCvs_Model_Cvs')->getConfigData('private_key');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('ecbankcvs')->__('繳費完成。'));
        $this->_order->save();
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('ecbankcvs')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('ecbankcvs')->getPendingPaymentStatus();
    }
	...
	protected function _cancelByError($message){
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('ecbankcvs')->__($message)
        )->save();
