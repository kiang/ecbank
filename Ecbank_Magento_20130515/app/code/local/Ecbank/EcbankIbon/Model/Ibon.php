...
class Ecbank_EcbankIbon_Model_Ibon extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'ecbankibon_ibon';
    ...
    {
		$actionUrl = Mage::getUrl('ecbankibon/processing/redirect');
		return $actionUrl;
    }
    ...
    public function sendParams()	//將參數與網址組合並用背景方式發送
    {
		...
		$urlParams = 'mer_id='.$value['mer_id'].'&payment_type='.$value['payment_type'].'&enc_key='.
		$value['enc_key'].'&od_sob='.$value['od_sob'].'&amt='.$value['amt'].'&prd_desc='.
		$value['prd_desc'].'&desc1='.$value['desc1'].'&desc2='.$value['desc2'].'&desc3='.
		$value['desc3'].'&ok_url='.$value['ok_url'];
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						...
						'amt'			=>	round($price),
						'prd_desc'		=>	$productName,
						'desc1'			=>	$this->getConfigData('des_1'),
						'desc2'			=>	$this->getConfigData('des_2'),
						'desc3'			=>	$this->getConfigData('des_3'),
						'ok_url'		=>  Mage::getUrl('ecbankibon/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('ibon API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('ibon API connection error. The request has not been processed.'));
		}
