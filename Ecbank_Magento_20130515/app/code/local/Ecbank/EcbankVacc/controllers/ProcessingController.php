...
class Ecbank_EcbankVacc_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'ecbankvacc/success';
    protected $_failureBlockType = 'ecbankvacc/failure';
    protected $_cancelBlockType = 'ecbankvacc/cancel';
    ...
    /**
     * when customer selects ecbankvacc payment method
     */
        ...
        try {        
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('ecbankvacc')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setVaccQuoteId($session->getQuoteId());
                $session->setVaccSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setVaccRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setVaccQuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * ecbankvacc returns POST variables to this action
     */
    ...
    /**
     * ecbankvacc return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsVaccRealOrderId();
            $session->setQuoteId($session->getVaccQuoteId(true));
            $session->setLastSuccessQuoteId($session->getVaccSuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getVaccQuoteId()) {
        ...
        $session->addError(Mage::helper('ecbankvacc')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from ecbankvacc/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as ecbankvacc-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$key = Mage::getModel('Ecbank_EcbankVacc_Model_Vacc')->getConfigData('private_key');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
		...
		//$paymentInfo = $this->_order()->getPayment()->getMethodInstance()->sendParams();
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('ecbankvacc')->__('繳費完成'));
        $this->_order->save();
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('ecbankvacc')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('ecbankvacc')->getPendingPaymentStatus();
    }
