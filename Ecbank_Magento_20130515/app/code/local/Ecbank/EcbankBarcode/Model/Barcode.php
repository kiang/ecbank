...
class Ecbank_EcbankBarcode_Model_Barcode extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'ecbankbarcode_barcode';
    ...
    protected $_paymentMethod			= 'barcode';
    ...
    {
		$actionUrl = Mage::getUrl('ecbankbarcode/processing/redirect');
		return $actionUrl;
    }
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						...
						'ok_url'		=>  Mage::getUrl('ecbankbarcode/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('barcode API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('barcode API connection error. The request has not been processed.'));
		}
