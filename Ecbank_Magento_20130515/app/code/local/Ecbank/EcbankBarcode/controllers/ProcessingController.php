...
class Ecbank_EcbankBarcode_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'ecbankbarcode/success';
    protected $_failureBlockType = 'ecbankbarcode/failure';
    protected $_cancelBlockType = 'ecbankbarcode/cancel';
    ...
    /**
     * when customer selects ecbankbarcode payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('ecbankbarcode')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setBarcodeQuoteId($session->getQuoteId());
                $session->setBarcodeSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setBarcodeRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setBarcodeQuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * ecbankbarcode returns POST variables to this action
     */
    ...
    /**
     * ecbankbarcode return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsBarcodeRealOrderId();
            $session->setQuoteId($session->getBarcodeQuoteId(true));
            $session->setLastSuccessQuoteId($session->getBarcodeSuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getBarcodeQuoteId()) {
        ...
        $session->addError(Mage::helper('ecbankbarcode')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from ecbankbarcode/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as ecbankbarcode-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$key = Mage::getModel('Ecbank_EcbankBarcode_Model_Barcode')->getConfigData('private_key');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('ecbankbarcode')->__('繳費完成。'));
        $this->_order->save();
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('ecbankbarcode')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('ecbankbarcode')->getPendingPaymentStatus();
    }
	...
	protected function _cancelByError($message){
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('ecbankbarcode')->__($message)
        )->save();
