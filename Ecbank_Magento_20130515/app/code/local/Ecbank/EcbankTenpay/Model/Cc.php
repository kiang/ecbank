...
class Ecbank_EcbankTenpay_Model_Cc extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'ecbanktenpay_cc';
    ...
    protected $_paymentMethod			= 'tenpay';	
    ...
    {
          return Mage::getUrl('ecbanktenpay/processing/redirect');
    }
    ...
    public function getFormFields() 
    {
    	...
    	$params = 	array(
						...
						'return_url'	=> Mage::getUrl('ecbanktenpay/processing/response'),
                                                                                                                   'ok_url'	=> Mage::getUrl('ecbanktenpay/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('tenpay API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('tenpay API connection error. The request has not been processed.'));
		}
