...
class Ecbank_EcbankTenpay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'ecbanktenpay/success';
    protected $_failureBlockType = 'ecbanktenpay/failure';
    protected $_cancelBlockType = 'ecbanktenpay/cancel';
    ...
    /**
     * when customer selects ecbanktenpay payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage::helper('ecbanktenpay')->__('轉向至Ecbank中，請稍候...')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setTenpayQuoteId($session->getQuoteId());
                $session->setTenpaySuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setTenpayRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setTenpayQuoteId($session->getQuoteId());
				//print_r($session);
				//exit;
                $session->clear();
            }
    ...
    /**
     * ecbanktenpay returns POST variables to this action
     */
    ...
    /**
     * ecbanktenpay return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsTenpayRealOrderId();
            $session->setQuoteId($session->getTenpayQuoteId(true));
            $session->setLastSuccessQuoteId($session->getTenpaySuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getTenpayQuoteId()) {
        ...
        $session->addError(Mage::helper('ecbanktenpay')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost()){
            ...
            // validate request ip coming from ecbanktenpay/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as ecbanktenpay-IP.');
        }*/
        ...
        if (empty($request)){
			...
			//Check transaction
		//驗證繳款完成通知是否由綠界傳來
		$key = Mage::getModel('Ecbank_EcbankTenpay_Model_Cc')->getConfigData('private_key');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    {   
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('ecbanktenpay')->__('繳費完成。'));
        $this->_order->save();
        $this->getResponse()->setBody(
            $this->getLayout()
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('ecbanktenpay')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('ecbanktenpay')->getPendingPaymentStatus();
    }
