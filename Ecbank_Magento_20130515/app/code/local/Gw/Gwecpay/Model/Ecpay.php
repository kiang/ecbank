...
class Gw_Gwecpay_Model_Ecpay extends Mage_Payment_Model_Method_Abstract

{
	...
	**/
    ...
    {
		$actionUrl = Mage::getUrl('gwecpay/processing/redirect');
		return $actionUrl;
    }
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						...
						//'LG'			=>	$price,
						//'Enn'			=>	$productName,
						'roturl'		=>  Mage::getUrl('gwecpay/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('ecpay API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('ecpay API connection error. The request has not been processed.'));
		}
