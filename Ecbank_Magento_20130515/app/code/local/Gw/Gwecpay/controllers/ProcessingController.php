...
class Gw_Gwecpay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'gwecpay/success';
    protected $_failureBlockType = 'gwecpay/failure';
    protected $_cancelBlockType = 'gwecpay/cancel';
    ...
    /**
     * when customer selects gwecpay payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('gwecpay')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setEcpayQuoteId($session->getQuoteId());
                $session->setEcpaySuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setEcpayRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setEcpayQuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * gwecpay returns POST variables to this action
     */
    ...
    /**
     * gwecpay return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsEcpayRealOrderId();
            $session->setQuoteId($session->getEcpayQuoteId(true));
            $session->setLastSuccessQuoteId($session->getEcpaySuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getEcpayQuoteId()) {
        ...
        $session->addError(Mage::helper('gwecpay')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from gwecpay/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as gwecpay-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$check_sum= $key = Mage::getModel('Gw_Gwecpay_Model_Ecpay')->getConfigData('check_code');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('gwecpay')->__('繳費完成。'));
        $this->_order->save();
        $this->getResponse()->setBody(
            $this->getLayout()
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('gwecpay')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('gwecpay')->getPendingPaymentStatus();
    }
	...
	protected function _cancelByError($message){
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('gwecpay')->__($message)
        )->save();
