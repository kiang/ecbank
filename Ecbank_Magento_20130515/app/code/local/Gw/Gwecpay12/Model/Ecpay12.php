...
class Gw_Gwecpay12_Model_Ecpay12 extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'gwecpay12_ecpay12';
    ...
    {
		$actionUrl = Mage::getUrl('gwecpay12/processing/redirect');
		return $actionUrl;
    }
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						...
						//'LG'			=>	$price,
						//'Enn'			=>	$productName,
						'roturl'		=>  Mage::getUrl('gwecpay12/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('ecpay12 API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('ecpay12 API connection error. The request has not been processed.'));
		}
