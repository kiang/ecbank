...
class Gw_Gwecpay12_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'gwecpay12/success';
    protected $_failureBlockType = 'gwecpay12/failure';
    protected $_cancelBlockType = 'gwecpay12/cancel';
    ...
    /**
     * when customer selects gwecpay12 payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('gwecpay12')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setEcpay12QuoteId($session->getQuoteId());
                $session->setEcpay12SuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setEcpay12RealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setEcpay12QuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * gwecpay12 returns POST variables to this action
     */
    ...
    /**
     * gwecpay12 return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsEcpay12RealOrderId();
            $session->setQuoteId($session->getEcpay12QuoteId(true));
            $session->setLastSuccessQuoteId($session->getEcpay12SuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getEcpay12QuoteId()) {
        ...
        $session->addError(Mage::helper('gwecpay12')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from gwecpay12/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as gwecpay12-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$check_sum= $key = Mage::getModel('Gw_Gwecpay12_Model_Ecpay12')->getConfigData('check_code');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('gwecpay12')->__('繳費完成。'));
        $this->_order->save();
        $this->getResponse()->setBody(
            $this->getLayout()
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('gwecpay12')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('gwecpay12')->getPendingPaymentStatus();
    }
	...
	protected function _cancelByError($message){
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('gwecpay12')->__($message)
        )->save();
