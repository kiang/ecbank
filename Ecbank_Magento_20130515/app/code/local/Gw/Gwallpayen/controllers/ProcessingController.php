...
class Gw_Gwallpayen_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'gwallpayen/success';
    protected $_failureBlockType = 'gwallpayen/failure';
    protected $_cancelBlockType = 'gwallpayen/cancel';
    ...
    /**
     * when customer selects gwallpayen payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('gwallpayen')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setAllpayenQuoteId($session->getQuoteId());
                $session->setAllpayenSuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setAllpayenRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setAllpayenQuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * gwallpayen returns POST variables to this action
     */
    ...
    /**
     * gwallpayen return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsAllpayenRealOrderId();
            $session->setQuoteId($session->getAllpayenQuoteId(true));
            $session->setLastSuccessQuoteId($session->getAllpayenSuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getAllpayenQuoteId()) {
        ...
        $session->addError(Mage::helper('gwallpayen')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from gwallpayen/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as gwallpayen-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$check_sum= $key = Mage::getModel('Gw_Gwallpayen_Model_Allpayen')->getConfigData('check_code');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('gwallpayen')->__('繳費完成。'));
        $this->_order->save();
        $this->getResponse()->setBody(
            $this->getLayout()
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('gwallpayen')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('gwallpayen')->getPendingPaymentStatus();
    }
	...
	protected function _cancelByError($message){
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('gwallpayen')->__($message)
        )->save();
