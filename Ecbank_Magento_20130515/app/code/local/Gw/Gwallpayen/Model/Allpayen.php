...
class Gw_Gwallpayen_Model_Allpayen extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'gwallpayen_allpayen';
    ...
    {
		$actionUrl = Mage::getUrl('gwallpayen/processing/redirect');
		return $actionUrl;
    }
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						'client'		=>	$this->getConfigData('shop_number'),
						'amount'		=>	round($price),
						'od_sob'		=>	$this->getOrder()->getRealOrderId(),
						//'LG'			=>	$price,
						//'Enn'			=>	$productName,
						'roturl'		=>  Mage::getUrl('gwallpayen/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('allpayen API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('allpayen API connection error. The request has not been processed.'));
		}
