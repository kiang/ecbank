...
class Gw_Gwallpay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    protected $_successBlockType = 'gwallpay/success';
    protected $_failureBlockType = 'gwallpay/failure';
    protected $_cancelBlockType = 'gwallpay/cancel';
    ...
    /**
     * when customer selects gwallpay payment method
     */
        ...
        try {
            ...
            if ($order->getState() != Mage_Sales_Model_Order::STATE_PENDING_PAYMENT) {
                $order->setState(
                    ...
                    Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                    Mage::helper('gwallpay')->__('轉向至繳費資訊頁面')
                )->save();
            ...
            if ($session->getQuoteId() && $session->getLastSuccessQuoteId()) {
                $session->setAllpayQuoteId($session->getQuoteId());
                $session->setAllpaySuccessQuoteId($session->getLastSuccessQuoteId());
                $session->setAllpayRealOrderId($session->getLastRealOrderId());
                $session->getQuote()->setIsActive(false)->save();
				$test = $session->getQuoteId();
				$test2 = $session->setAllpayQuoteId($session->getQuoteId());
                $session->clear();
            }
    ...
    /**
     * gwallpay returns POST variables to this action
     */
    ...
    /**
     * gwallpay return action
     */
    public function successAction()
    {
        try {
            $session = $this->_getCheckout();
            $session->unsAllpayRealOrderId();
            $session->setQuoteId($session->getAllpayQuoteId(true));
            $session->setLastSuccessQuoteId($session->getAllpaySuccessQuoteId(true));
    ...
    public function cancelAction()
    {
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getAllpayQuoteId()) {
        ...
        $session->addError(Mage::helper('gwallpay')->__('The order has been canceled.'));
        $this->_redirect('checkout/cart');
    }
    ...
    {	
        ...
        if (!$this->getRequest()->isPost())
            ...
            // validate request ip coming from gwallpay/RBS subnet
        ...
        /*if (substr($remoteAddr,0,11) != '155.136.16.') {
            Mage::throwException('IP can\'t be validated as gwallpay-IP.');
        }*/
        ...
        if (empty($request))
			...
			//Check transaction 
		//驗證繳款完成通知是否由綠界傳來
		$check_sum= $key = Mage::getModel('Gw_Gwallpay_Model_Allpay')->getConfigData('check_code');	//在controller須先呼叫model cvs後才能使用getConfigData()
    ...
    protected function _processSale($request)
    {
        ...
        $this->_order->addStatusToHistory($this->_paymentInst->getConfigData('order_status'), Mage::helper('gwallpay')->__('繳費完成。'));
        $this->_order->save();
        $this->getResponse()->setBody(
            $this->getLayout()
    ...
    protected function _processCancel($request)
    {
        ...
        if ($this->_order->canCancel()) {
            $this->_order->cancel();
            $this->_order->addStatusToHistory(Mage_Sales_Model_Order::STATE_CANCELED, Mage::helper('gwallpay')->__('Payment was canceled'));
            $this->_order->save();
        }
    ...
    {
        return Mage::helper('gwallpay')->getPendingPaymentStatus();
    }
	...
	protected function _cancelByError($message){
		...
		$order->setState(
            ...
            Mage_Sales_Model_Order::STATE_CANCELED,
            Mage::helper('gwallpay')->__($message)
        )->save();
