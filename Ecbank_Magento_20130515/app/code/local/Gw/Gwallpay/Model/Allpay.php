...
class Gw_Gwallpay_Model_Allpay extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'gwallpay_allpay';
    ...
    {
		$actionUrl = Mage::getUrl('gwallpay/processing/redirect');
		return $actionUrl;
    }
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						...
						//'LG'			=>	$price,
						//'Enn'			=>	$productName,
						'roturl'		=>  Mage::getUrl('gwallpay/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('allpay API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('allpay API connection error. The request has not been processed.'));
		}
