...
class Gw_Gwecpay3_Model_Ecpay3 extends Mage_Payment_Model_Method_Abstract

{
	...
	* @var string [a-z0-9_]
	**/
	protected $_code = 'gwecpay3_ecpay3';
    ...
    {
		$actionUrl = Mage::getUrl('gwecpay3/processing/redirect');
		return $actionUrl;
    }
    ...
    public function setParams()
    {
    	...
    	$params = 	array(
						...
						//'LG'			=>	$price,
						//'Enn'			=>	$productName,
						'roturl'		=>  Mage::getUrl('gwecpay3/processing/response')
    				);
    	return $params;
    }
	...
	protected function processAdminRequest($params, $requestTimeout = 60)
	{
		try {
			...
			if (empty($responseBody))
				Mage::throwException($this->_getHelper()->__('ecpay3 API failure. The request has not been processed.'));
			// create array out of response

		} catch (Exception $e) {
            Mage::log($e->getMessage());
			Mage::throwException($this->_getHelper()->__('ecpay3 API connection error. The request has not been processed.'));
		}
