 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_ECPAY_","greenworld_ecpay"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_ECPAY_","綠界－線上刷卡繳費"); 
define("_iMODULE_DESCRUPTION_ECPAY_","提供管理者在Prestashop網站上，使用綠界科技的線上刷卡繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_ECPAY__","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_ECPAY_","使用線上刷卡繳費"); 
define("_iMODULE_VERSION_ECPAY_","1.0.0"); 
class greenworld_ecpay extends PaymentModule {
    private $_html='';
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_ECPAY_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_ECPAY_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_ECPAY_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_ECPAY_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_ECPAY_);
       ...
       */
        $this->BaseURL="https://ecpay.com.tw/form_Sc_to5.php";         
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue($this->name.'_shop_check_code', Tools::getValue('shop_check_code'),true);
        ...
        if(!Configuration::get($this->name.'_shop_check_code')==null){
            $this->_shop_check_code=Configuration::get($this->name.'_shop_check_code');
        }else{
     ...
     public function hookPayment($params){
        global $smarty;
        $smarty->assign('payment',$this->l(_iMODULE_FIRST_TITLE_ECPAY_));    
    ...
    public function getShopCheckCode(){
        $tempReturn="";
        if(Configuration::get($this->name.'_shop_check_code') ){
            $tempReturn=Configuration::get($this->name.'_shop_check_code');
        }else{
