...
include_once(dirname(__FILE__).'/greenworld_webatm.php');
include_once(dirname(__FILE__).'/../../classes/OrderHistory.php');

session_start();

global $cookie;
$CheckPay=new greenworld_webatm();
$authorized = false;
...
if(!session_is_registered("checkStep")){
    ...
    if ($total==0){
        echo Module::display(_iMODULE_NAME_ATM_,'ErrorStep.tpl');
        session_destroy(); 
    }else{
        $_SESSION["checkStep"]="1";
        echo Module::display(_iMODULE_NAME_ATM_,'validationOrder.tpl');
    }
    ...
    echo Module::display(_iMODULE_NAME_ATM_,'sentToWebATM.tpl');
}else{
    echo Module::display(_iMODULE_NAME_ATM_,'ErrorStep.tpl');
    session_unregister("checkStep");
    session_destroy();
