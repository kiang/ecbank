...
// class methods
    ...
    function process_button() {
	  ...
	  if($price <=25) {
			echo "<Script Language=Javascript>
					alert('歐付寶 ECBank 虛擬帳號付款, 最低付款金額為 25 元,按下確定後會返回購物車');
					location(history(-1));
				  </script>";
				  exit;
		} 
  ...
  function get_error() {
	...
	function response() {		
		...
		$checkcode = '';
		$checkcode_sql =  tep_db_query( "select configuration_value checkcode from configuration where configuration_key = 'MODULE_PAYMENT_ECBANK_VACC_CHECKCODE'");
		$checkcode_array = tep_db_fetch_array($checkcode_sql);
		$checkcode = $checkcode_array['checkcode'];
		
		$orders_status = '2'; //改變的訂單狀態依商家實際情況做修正
		/*
		$orders_status_sql = tep_db_query( "select configuration_value orders_status from configuration where configuration_key = 'MODULE_PAYMENT_ECBANK_VACC_ORDER_STATUS_ID'");
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶ECbank-虛擬帳號 付款 結帳的顯示順序',	 'MODULE_PAYMENT_ECBANK_VACC_SORT_ORDER', 		 '0', 							'歐付寶 ECBank -虛擬帳號 付款結帳的顯示順序，數字小者在前', 					 	'6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('歐付寶ECBank-虛擬帳號 付款 結帳預設定單狀態', 'MODULE_PAYMENT_ECBANK_VACC_ORDER_STATUS_ID', '0', 							'設定使用歐付寶 ECBank -虛擬帳號 付款卡 結帳時，預設的訂單狀態',	'6', '1', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('歐付寶ECBank-虛擬帳號 付款 結帳地區', 				 'MODULE_PAYMENT_ECBANK_VACC_ZONE', 					 '0', 							'如果選擇地區，則只有該地區可以使用這個結帳方式', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) 							 values ('啟動 歐付寶ECBank-虛擬帳號 付款模組', 			 'MODULE_PAYMENT_ECBANK_VACC_STATUS', 				 'True', 						'接受 <a target=_BLANK href=http://www.ecbank.com.tw><font color=green>歐付寶ECBank-虛擬帳號付款</font></a> 結帳？', 											'6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶ECBank-虛擬帳號 付商家編號', 						 'MODULE_PAYMENT_ECBANK_VACC_MID', 						 '歐付寶商店代號', 		'請輸入 歐付寶 ECBank -虛擬帳號 付款 商家編號', 											'6', '4', now())");
      //tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶 虛擬帳號 付款 完成回傳網址', 		 'MODULE_PAYMENT_ECBANK_VACC_RETUREURL', 		 	 'http://OSC網站網址/checkout_process.php', 					'請輸入完成刷卡後將要回傳的網址.', 								'6', '5', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('回傳查核設定', 									 'MODULE_PAYMENT_ECBANK_VACC_CHECKCODE',			 '商家檢查碼','填歐付寶 ECbank -虛擬帳號付款的商家檢查碼,可避免偽冒', 							'6', '6', now())");
