...
// class methods
   ...
   //   $_SESSION['comments'] .= " ECBank 自訂訂單編號:".$ordnum."";      
    ...
    function before_process() {   	
		...
		$checkcode = '';
		$checkcode_sql =  tep_db_query( "select configuration_value checkcode from configuration where configuration_key = 'MODULE_PAYMENT_ECBANK_PINCODE_CHECKCODE'");
		$checkcode_array = tep_db_fetch_array($checkcode_sql);
		$checkcode = $checkcode_array['checkcode'];
		
		$orders_status = '2';  //改變的訂單狀態依商家實際情況做修正
		/*
		$orders_status_sql = tep_db_query( "select configuration_value orders_status from configuration where configuration_key = 'MODULE_PAYMENT_ECBANK_PINCODE_ORDER_STATUS_ID'");
    ...
    function install() {
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶ECBank-超商代碼支付 付商家編號', 						 'MODULE_PAYMENT_ECBANK_PINCODE_MID', 						 '歐付寶商店代號', 		'請輸入 歐付寶 ECBank -超商代碼支付 商家編號', 											'6', '4', now())");
