...
// class constructor
    function ecpay6() {
      ...
      $this->code = 'ecpay6';
      $this->title = MODULE_PAYMENT_ECPAY6_TEXT_TITLE;
      $this->description = MODULE_PAYMENT_ECPAY6_TEXT_DESCRIPTION;
      $this->sort_order = MODULE_PAYMENT_ECPAY6_SORT_ORDER;
      $this->enabled = ((MODULE_PAYMENT_ECPAY6_STATUS == 'True') ? true : false);
      
      if ((int)MODULE_PAYMENT_ECPAY6_ORDER_STATUS_ID > 0) {
        $this->order_status = MODULE_PAYMENT_ECPAY6_ORDER_STATUS_ID;
      }
...
// class methods
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_ECPAY6_ZONE > 0) ) {
        $check_flag = false;
        $check_query = tep_db_query("select zone_id from " . TABLE_ZONES_TO_GEO_ZONES . " where geo_zone_id = '" . MODULE_PAYMENT_ECPAY6_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
    ...
    function confirmation() {
       return array('title' => MODULE_PAYMENT_ECPAY6_TEXT_CONFIRMATION);
    }
    ...
    function process_button() {
      ...
      $process_button_string = 	tep_draw_hidden_field('client', MODULE_PAYMENT_ECPAY6_ID ) .	
    ...
    //$loginName=MODULE_PAYMENT_ECPAY_CHECKCODE;
		$checkcode_sql =  tep_db_query( "select configuration_value checkcode from configuration where configuration_key = 'MODULE_PAYMENT_ECPAY6_CHECKCODE'");
	...
	function get_error() {
	  	switch ($_GET['error']) {
		  		case 1 :
		  				$error_message = MODULE_PAYMENT_ECPAY6_TEXT_ERROR_1 ;
		  				break ;
		 		 case 2 :
		  				$error_message = MODULE_PAYMENT_ECPAY6_TEXT_ERROR_2 ;
		  				break ;
		  		case 3 :
		  				$error_message = MODULE_PAYMENT_ECPAY6_TEXT_ERROR_3 ;
		 					 break ;
	  	}
      $error = array('title' => MODULE_PAYMENT_ECPAY6_TEXT_ERROR,
                     'error' => $error_message );
      return $error;
    }
    ...
    function check() {
      if (!isset($this->_check)) {
        $check_query = tep_db_query("select configuration_value from " . TABLE_CONFIGURATION . " where configuration_key = 'MODULE_PAYMENT_ECPAY6_STATUS'");
    ...
    function install() {
    	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶ECPAY刷卡 分期付款-6期 結帳的顯示順序',	 'MODULE_PAYMENT_ECPAY6_SORT_ORDER', 		 '0', 							'ECPAY 刷卡 分期付款-6期，結帳的顯示順序，數字小者在前', 					 	'6', '0', now())");
    	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('歐付寶ECPAY刷卡 分期付款-6期 結帳預設定單狀態', 'MODULE_PAYMENT_ECPAY6_ORDER_STATUS_ID', '0', 							'設定使用歐付寶ECPAY刷卡 分期付款-6期 結帳時，預設的訂單狀態',	'6', '1', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
    	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('歐付寶ECPAY刷卡 分期付款-6期 結帳地區', 				 'MODULE_PAYMENT_ECPAY6_ZONE', 					 '0', 							'如果選擇地區，則只有該地區可以使用這個結帳方式', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) 							 values ('啟動 歐付寶ECPAY 分期付款-6期 刷卡模組', 			 'MODULE_PAYMENT_ECPAY6_STATUS', 				 'True', 						'接受 <a href=http://www.gwpay.com.tw target=_BLANK><font color=green>歐付寶ECPAY刷卡 分期付款-6期</font></a> 結帳？', 											'6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶ECPAY 分期付款商家編號', 						 'MODULE_PAYMENT_ECPAY6_ID', 						 '歐付寶商店代號', 		'請輸入 歐付寶ECPAY 分期付款商家編號', 											'6', '4', now())");
      ...
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('回傳查核設定', 									 'MODULE_PAYMENT_ECPAY6_CHECKCODE',			 '交易驗證碼','填上歐付寶金流的管理帳號,可避免偽冒', 							'6', '6', now())");
    ...
    function keys() {
      return array('MODULE_PAYMENT_ECPAY6_STATUS', 'MODULE_PAYMENT_ECPAY6_ID', 'MODULE_PAYMENT_ECPAY6_ZONE', 'MODULE_PAYMENT_ECPAY6_ORDER_STATUS_ID', 'MODULE_PAYMENT_ECPAY6_SORT_ORDER','MODULE_PAYMENT_ECPAY6_CHECKCODE');
    }
  }
?>
