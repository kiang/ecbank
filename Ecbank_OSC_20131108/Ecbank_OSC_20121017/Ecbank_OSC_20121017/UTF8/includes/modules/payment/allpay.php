...
// class constructor
	...
	$this->form_action_url = 'https://credit.allpay.com.tw/form_Sc_to5.php '; //中文頁面 Chinese  PayPage
    }
...
// class methods
	...
	$_SESSION['comments'] .= " ALLPAY 自訂訂單編號:" . $ordnum . "";
	// if ( MODULE_PAYMENT_PAYONLIE_RETUREURL != '') {$reurl = 'returl' ; $returl_var= MODULE_PAYMENT_ALLPAY_RETUREURL ; }  
	...
	$process_button_string = tep_draw_hidden_field('client', MODULE_PAYMENT_ALLPAY_ID) .
	...
	//$loginName=MODULE_PAYMENT_ALLPAY_CHECKCODE;
	$checkcode_sql = tep_db_query("select configuration_value checkcode from configuration where configuration_key = 'MODULE_PAYMENT_ALLPAY_CHECKCODE'");
	...
	if ($my_spcheck == $_POST['spcheck']) {
		...
		$result_msg = '交易成功，ALLPAY 交易單號：' . $_REQUEST['gwsr'] . '，處理日期時間' . $_REQUEST['process_date'] . $_REQUEST['process_time'];
	...
	switch ($_GET['error']) {
	    case 1 :
		$error_message = MODULE_PAYMENT_ALLPAY_TEXT_ERROR_1;
		break;
	    case 2 :
		$error_message = MODULE_PAYMENT_ALLPAY_TEXT_ERROR_2;
		break;
	    case 3 :
		$error_message = MODULE_PAYMENT_ALLPAY_TEXT_ERROR_3;
		break;
	}
	$error = array('title' => MODULE_PAYMENT_ALLPAY_TEXT_ERROR,
	    'error' => $error_message);
	return $error;
    }
    ...
    function install() {
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('歐付寶ALLPAY刷卡 結帳的顯示順序',	 'MODULE_PAYMENT_ALLPAY_SORT_ORDER', 		 '0', 							'ALLPAY 結帳的顯示順序，數字小者在前', 					 	'6', '0', now())");
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values (' 歐付寶ALLPAY刷卡 結帳預設定單狀態', 'MODULE_PAYMENT_ALLPAY_ORDER_STATUS_ID', '0', 							'設定使用  歐付寶ALLPAY刷卡 結帳時，預設的訂單狀態',	'6', '1', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ( 歐付寶ALLPAY刷卡 結帳地區', 				 'MODULE_PAYMENT_ALLPAY_ZONE', 					 '0', 							'如果選擇地區，則只有該地區可以使用這個結帳方式', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) 							 values ('啟動  歐付寶ALLPAY 刷卡模組', 			 'MODULE_PAYMENT_ALLPAY_STATUS', 				 'True', 						'接受 <a href=http://www.gwpay.com.tw target=_BLANK><font color=green> 歐付寶ALLPAY刷卡</font></a> 結帳？', 											'6', '3', 'tep_cfg_select_option(array(\'True\', \'False\'), ', now())");
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values (' 歐付寶ALLPAY商家編號', 						 'MODULE_PAYMENT_ALLPAY_ID', 						 '商店代號', 		'請輸入 歐付寶ALLPAY 商家編號', 											'6', '4', now())");
	//tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('ALLPAY刷卡 完成回傳網址', 		 'MODULE_PAYMENT_ALLPAY_RETUREURL', 		 	 'http://OSC網站網址/checkout_process.php', 					'請輸入完成刷卡後將要回傳的網址.', 								'6', '5', now())");
	tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) 														 values ('回傳查核設定', 									 'MODULE_PAYMENT_ALLPAY_CHECKCODE',			 '商家檢查碼', 							'6', '6', now())");
    }
    ...
    function keys() {
	return array('MODULE_PAYMENT_ALLPAY_STATUS', 'MODULE_PAYMENT_ALLPAY_ID', 'MODULE_PAYMENT_ALLPAY_ZONE', 'MODULE_PAYMENT_ALLPAY_ORDER_STATUS_ID', 'MODULE_PAYMENT_ALLPAY_SORT_ORDER', 'MODULE_PAYMENT_ALLPAY_CHECKCODE');
    }
