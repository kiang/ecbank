 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_ECPAY12_","greenworld_ecpay12"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_ECPAY12_","綠界－ECPAY分12期繳費"); 
define("_iMODULE_DESCRUPTION_ECPAY12_","提供管理者在Prestashop網站上，使用綠界科技的ECPAY分12期繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_ECPAY12_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_ECPAY12_","使用ECPAY12_"); 
define("_iMODULE_VERSION_ECPAY12_","1.0.0"); 
class greenworld_ecpay12 extends PaymentModule {
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_ECPAY12_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_ECPAY12_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_ECPAY12_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_ECPAY12_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_ECPAY12_);    }
    ...
    public function getEncryptionCode(){
        $encryption_code=Configuration::get('gw_ecpay12_encryption');
    ...
    public function uninstall(){
        if (!Configuration::deleteByName('gw_ecpay12_encryption')
            OR !Configuration::deleteByName('gw_ecpay12_mer_id')
            OR !parent::uninstall())
            return false;
    }
    private function _displayForm(){
        $mer_id=Configuration::get('gw_ecpay12_mer_id');
        $encryption_code=Configuration::get('gw_ecpay12_encryption');
        $this->_html .= '  
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_ecpay12_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_ecpay12_encryption', Tools::getValue('encryption_code'),true);
     ...
     public function hookPaymentReturn($params){
        ...
        $smarty->assign(array(
            ...
            'mer_id'=> Configuration::get('gw_ecpay3_mer_id'),
