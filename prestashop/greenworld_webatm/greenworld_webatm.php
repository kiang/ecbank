 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_WEBATM_","greenworld_webatm"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_WEBATM_","綠界－WEBATM_繳費"); 
define("_iMODULE_DESCRUPTION_WEBATM_","提供管理者在Prestashop網站上，使用綠界科技的WEBATM_繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_WEBATM_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_WEBATM_","使用WEBATM_"); 
define("_iMODULE_VERSION_WEBATM_","1.0.0"); 
class greenworld_webatm extends PaymentModule {
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_WEBATM_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_WEBATM_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_WEBATM_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_WEBATM_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_WEBATM_);    }
    ...
    public function uninstall(){
        if (!Configuration::deleteByName('gw_webatm_encryption')
            OR !Configuration::deleteByName('gw_webatm_mer_id')
            OR !parent::uninstall())
            return false;
    }
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_webatm_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_webatm_encryption', Tools::getValue('encryption_code'),true);
