 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_PAYPAL_","greenworld_paypal"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_PAYPAL_","綠界－PAYPAL繳費"); 
define("_iMODULE_DESCRUPTION_PAYPAL_","提供管理者在Prestashop網站上，使用綠界科技的PAYPAL繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_PAYPAL_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_PAYPAL_","使用PAYPAL_"); 
define("_iMODULE_VERSION_PAYPAL_","1.0.0"); 
class greenworld_paypal extends PaymentModule {
    private $_html='';
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_PAYPAL_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_PAYPAL_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_PAYPAL_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_PAYPAL_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_PAYPAL_);    }
    ...
    public function getEncryptionCode(){
        $encryption_code=Configuration::get('gw_paypal_encryption');
    ...
    public function uninstall(){
        if (!Configuration::deleteByName('gw_paypal_encryption')
            OR !Configuration::deleteByName('gw_paypal_mer_id')
            OR !parent::uninstall())
            return false;
    }
    private function _displayForm(){
        $mer_id=Configuration::get('gw_paypal_mer_id');
        $encryption_code=Configuration::get('gw_paypal_encryption');
        $this->_html .= '  
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_paypal_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_paypal_encryption', Tools::getValue('encryption_code'),true);
