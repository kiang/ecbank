 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_ECPAY6_","greenworld_ecpay6"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_ECPAY6_","綠界－ECPAY分6期繳費"); 
define("_iMODULE_DESCRUPTION_ECPAY6_","提供管理者在Prestashop網站上，使用綠界科技的ECPAY分6期繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_ECPAY6_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_ECPAY6_","使用ECPAY6_"); 
define("_iMODULE_VERSION_ECPAY6_","1.0.0"); 
class greenworld_ecpay6 extends PaymentModule {
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_ECPAY6_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_ECPAY6_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_ECPAY6_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_ECPAY6_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_ECPAY6_);    }
    ...
    public function getEncryptionCode(){
        $encryption_code=Configuration::get('gw_ecpay6_encryption');
    ...
    public function uninstall(){
        if (!Configuration::deleteByName('gw_ecpay6_encryption')
            OR !Configuration::deleteByName('gw_ecpay6_mer_id')
            OR !parent::uninstall())
            return false;
    }
    private function _displayForm(){
        $mer_id=Configuration::get('gw_ecpay6_mer_id');
        $encryption_code=Configuration::get('gw_ecpay6_encryption');
        $this->_html .= '  
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_ecpay6_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_ecpay6_encryption', Tools::getValue('encryption_code'),true);
