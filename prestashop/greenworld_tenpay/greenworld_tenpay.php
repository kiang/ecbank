 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_TENPAY_","greenworld_tenpay"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_TENPAY_","綠界－財付通繳費"); 
define("_iMODULE_DESCRUPTION_TENPAY_","提供管理者在Prestashop網站上，使用綠界科技的財付通繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_TENPAY_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_TENPAY_","使用TENPAY_"); 
define("_iMODULE_VERSION_TENPAY_","1.0.0"); 
class greenworld_tenpay extends PaymentModule {
    private $_html='';
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_TENPAY_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_TENPAY_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_TENPAY_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_TENPAY_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_TENPAY_);    }
    ...
    public function getEncryptionCode(){
        $encryption_code=Configuration::get('gw_tenpay_encryption');
        return $encryption_code;
    }
	
	
    public function uninstall(){
        if (!Configuration::deleteByName('gw_tenpay_encryption')
            OR !Configuration::deleteByName('gw_tenpay_mer_id')
            OR !parent::uninstall())
            return false;
    }
    ...
    private function _displayForm(){
        $mer_id=Configuration::get('gw_tenpay_mer_id');
        $encryption_code=Configuration::get('gw_tenpay_encryption');
        $this->_html .= '  
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_tenpay_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_tenpay_encryption', Tools::getValue('encryption_code'),true);
