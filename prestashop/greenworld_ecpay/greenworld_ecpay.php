...
define("_iMODULE_CHINESE_NAME_ECPAY_","綠界－ECPAY繳費"); 
define("_iMODULE_DESCRUPTION_ECPAY_","提供管理者在Prestashop網站上，使用綠界科技的ECPAY繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_ECPAY_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_ECPAY_","使用ECPAY_"); 
...
class greenworld_ecpay extends PaymentModule {
    ...
    public function getEncryptionCode(){
        $encryption_code=Configuration::get('gw_ecpay_encryption');
        return $encryption_code;
    }
    public function uninstall(){
        if (!Configuration::deleteByName('gw_ecpay_encryption')
            OR !Configuration::deleteByName('gw_ecpay_mer_id')
            OR !parent::uninstall())
            return false;
    }
    ...
    private function _displayForm(){
        $mer_id=Configuration::get('gw_ecpay_mer_id');
        $encryption_code=Configuration::get('gw_ecpay_encryption');
        $this->_html .= '  
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_ecpay_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_ecpay_encryption', Tools::getValue('encryption_code'),true);
