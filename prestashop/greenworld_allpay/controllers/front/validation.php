...
class GreenWorld_AllpayValidationModuleFrontController extends ModuleFrontController
{
	public function postProcess()
	{
		...
		foreach (Module::getPaymentModules() as $module)
			if ($module['name'] == 'greenworld_allpay')
			{
				$authorized = true;
				break;
			}
		...
		$this->module->validateOrder((int)$cart->id, 1, $inttotal, $this->module->displayName, null, array(), null, (int)$currency->id, $customer->secure_key);
              ...
              /*  $mer_id=Configuration::get('gw_webatm_mer_id');
               ...
               /* if(!isset($res['error']) || $res['error'] != '0'){
                    ...
                    $finishURL='index.php?controller=order-confirmation&id_cart='.(int)$cart->id.'&id_module='.(int)$this->module->id.'&id_order='.$this->module->currentOrder.'&key='.$customer->secure_key.'&amt='.$inttotal.'&return_url='.$return_url.'.php&mer_id='.Configuration::get('gw_allpay_mer_id');
