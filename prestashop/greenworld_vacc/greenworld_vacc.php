 ...
 * @author 李泓承 
 */
define("_iMODULE_NAME_VACC_","greenworld_vacc"); //設定 Class的name。
define("_iMODULE_CHINESE_NAME_VACC_","綠界－虛擬帳戶繳費"); 
define("_iMODULE_DESCRUPTION_VACC_","提供管理者在Prestashop網站上，使用綠界科技的虛擬帳戶繳費"); 
define("_iMODULE_UNINSTALL_MESSAGE_VACC_","Are you sure you want to delete your details ?"); 
define("_iMODULE_FIRST_TITLE_VACC_","使用虛擬帳戶"); 
define("_iMODULE_VERSION_VACC_","1.0.0"); 
class greenworld_vacc extends PaymentModule {
    ...
    function __construct(){  
        $this->name = _iMODULE_NAME_VACC_;
        
        parent::__construct();
        $this->path=$this->_path;
        $this->tab = 'payments_gateways';  
        $this->version = _iMODULE_VERSION_VACC_;  
        $this->displayName = $this->l(_iMODULE_CHINESE_NAME_VACC_);  
        $this->description = $this->l(_iMODULE_DESCRUPTION_VACC_);
        $this->confirmUninstall = $this->l(_iMODULE_UNINSTALL_MESSAGE_VACC_);    }
    ...
    public function uninstall(){
        if (!Configuration::deleteByName('gw_vacc_encryption')
            OR !Configuration::deleteByName('gw_vacc_mer_id')
            OR !parent::uninstall())
            return false;
    }
    ...
    public function getContent(){
        if (Tools::isSubmit('submit')){  
            if(!sizeof ($this->_postErrors)){
                ...
                Configuration::updateValue('gw_vacc_mer_id', Tools::getValue('shop_code'),true);
                Configuration::updateValue('gw_vacc_encryption', Tools::getValue('encryption_code'),true);
