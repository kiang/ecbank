	...
	protected function index() {
		...
		$products = $this->cart->getProducts();
               ...
               foreach($products as $p){
				...
				$goods_name = $p['name'].'_';
				foreach($p['option'] as $op){ //20120921 新增品號加長項目品號  品名加長項目品名
					$goods_id .= $op['product_option_value_id'];
					$goods_name .= $op['option_value'].'_';
				}
				$goods_id = substr($goods_id,0,7);
				$goods_name = substr($goods_name,0,50);
		...
		$this->data['def_url'] .= "<input type='hidden' name='ok_url' value='".$return_url."'>";
                foreach($products as $p){
					$goods_id = $p['product_id'];
					foreach($p['option'] as $op){ //20120921 
					$goods_id .= $op['product_option_value_id'];
					}
				$goods_id = substr($goods_id,0,7);
                    $this->data['def_url'] .= "<input type='hidden' name='goods_name[]' value='".$goods_id."'>";
                ...
                if($i_invoice == 'yes'){ //電子發票
                    ...
                    $this->data['def_url'] .= "<input type='hidden' name='inv_amt' value='".$amt."'>";
                    ...
                    foreach($products as $p){
						$goods_name = $p['name'].'_';
						foreach($p['option'] as $op){ //20120921 
							$goods_name .= $op['name'].'_';
						}
                        $this->data['def_url'] .= "<input type='hidden' name='prd_name[]' value='".$goods_name."'>";
