...
class ControllerPaymentAllpayCredit30 extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/allpay_credit30');
        ...
        $hash_iv = $this->config->get('allpay_credit30_iv_key');
        $hash_key = $this->config->get('allpay_credit30_hash_key');
        $merchant_id = $this->config->get('allpay_credit30_account');
        $chose_payment = "Credit";
        $trade_date = date("Y/m/d H:i:s");
        $total_amt = intval(round($order_info['total']));
        $return_url = $this->url->link('payment/allpay_credit30/callback');
        ...
        $input_array = array(
            "MerchantID" => $this->config->get('allpay_credit30_account'),
			...
			"CreditInstallment"=>"30",
			"InstallmentAmount"=>$total_amt,
			"Redeem"=>"Y",
        );
        ...
        $this->data['allpay_credit30_description'] = nl2br($this->config->get('allpay_credit30_description_' . $this->config->get('config_language_id')));
        $this->data['continue'] = $this->url->link('checkout/allpay_credit30_success');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/allpay_credit30.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/allpay_credit30.tpl';
        } else {
            $this->template = 'default/template/payment/allpay_credit30.tpl';
        }
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('allpay_credit_order_status_id'), "");
    ...
    public function confirm() {
        $this->load->model('checkout/order');
        $this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('allpay_credit30_order_status_id'), "");
        $this->cart->clear();
    }
    ...
    public function callback() {
        ...
        $mer_id = $this->config->get('allpay_credit30_account');
        $order_finish_statu = $this->config->get('allpay_credit30_order_finish_status_id');
