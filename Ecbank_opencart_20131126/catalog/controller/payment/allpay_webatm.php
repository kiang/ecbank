...
class ControllerPaymentAllpayWebatm extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/allpay_webatm');
        ...
        $hash_iv = $this->config->get('allpay_webatm_iv_key');
        $hash_key = $this->config->get('allpay_webatm_hash_key');
        $merchant_id = $this->config->get('allpay_webatm_account');
        $chose_payment = "WebATM";
        $trade_date = date("Y/m/d H:i:s");
        $total_amt = intval(round($order_info['total']));
        $return_url = $this->url->link('payment/allpay_webatm/callback');
        ...
        $input_array = array(
            "MerchantID" => $this->config->get('allpay_webatm_account'),
        ...
        $this->data['allpay_webatm_description'] = nl2br($this->config->get('allpay_webatm_description_' . $this->config->get('config_language_id')));
        $this->data['continue'] = $this->url->link('checkout/allpay_webatm_success');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/allpay_webatm.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/allpay_webatm.tpl';
        } else {
            $this->template = 'default/template/payment/allpay_webatm.tpl';
        }
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('allpay_webatm_order_status_id'), "");
    ...
    public function callback() {
        ...
        $mer_id = $this->config->get('allpay_webatm_account');
        $order_finish_statu = $this->config->get('allpay_webatm_order_finish_status_id');
