...
class ControllerPaymentAllpayCvs extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/allpay_cvs');
        ...
        $hash_iv = $this->config->get('allpay_cvs_iv_key');
        $hash_key = $this->config->get('allpay_cvs_hash_key');
        $merchant_id = $this->config->get('allpay_cvs_account');
        $chose_payment = "CVS";
        $trade_date = date("Y/m/d H:i:s");
        $total_amt = intval(round($order_info['total']));
        $return_url = $this->url->link('payment/allpay_cvs/callback');
        ...
        $input_array = array(
            "MerchantID" => $this->config->get('allpay_cvs_account'),
        ...
        $this->data['allpay_cvs_description'] = nl2br($this->config->get('allpay_cvs_description_' . $this->config->get('config_language_id')));
        $this->data['continue'] = $this->url->link('checkout/allpay_cvs_success');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/allpay_cvs.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/allpay_cvs.tpl';
        } else {
            $this->template = 'default/template/payment/allpay_cvs.tpl';
        }
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('allpay_cvs_order_status_id'), "");
    ...
    public function callback() {
        ...
        $mer_id = $this->config->get('allpay_cvs_account');
        $order_finish_statu = $this->config->get('allpay_cvs_order_finish_status_id');
