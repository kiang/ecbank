...
class ControllerPaymentAllpayAlipay extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/allpay_alipay');
        ...
        $hash_iv = $this->config->get('allpay_alipay_iv_key');
        $hash_key = $this->config->get('allpay_alipay_hash_key');
        $merchant_id = $this->config->get('allpay_alipay_account');
        $chose_payment = "Alipay";
        $trade_date = date("Y/m/d H:i:s");
        $total_amt = intval(round($order_info['total']));
        $return_url = $this->url->link('payment/allpay_alipay/callback');
		...
		//$email="";
        $input_array = array(
            "MerchantID" => $this->config->get('allpay_alipay_account'),
			...
			"AlipayItemName"=>$item_name,
			"AlipayItemCounts"=>"1",
			"AlipayItemPrice"=>$total_amt,
			"PhoneNo"=>$phone,
			"UserName"=>$name,
			"Email"=>$email,
        );
        ...
        $this->data['allpay_alipay_description'] = nl2br($this->config->get('allpay_alipay_description_' . $this->config->get('config_language_id')));
        $this->data['continue'] = $this->url->link('checkout/allpay_alipay_success');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/allpay_alipay.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/allpay_alipay.tpl';
        } else {
            $this->template = 'default/template/payment/allpay_alipay.tpl';
        }
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('allpay_alipay_order_status_id'), "");
    ...
    public function callback() {
        ...
        $mer_id = $this->config->get('allpay_alipay_account');
        $order_finish_statu = $this->config->get('allpay_alipay_order_finish_status_id');
