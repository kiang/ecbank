...
class ControllerPaymentAllpayAtm extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/allpay_atm');
        ...
        $hash_iv = $this->config->get('allpay_atm_iv_key');
        $hash_key = $this->config->get('allpay_atm_hash_key');
        $merchant_id = $this->config->get('allpay_atm_account');
        $chose_payment = "ATM";
        $trade_date = date("Y/m/d H:i:s");
        $total_amt = intval(round($order_info['total']));
        $return_url = $this->url->link('payment/allpay_atm/callback');
        ...
        $input_array = array(
            "MerchantID" => $this->config->get('allpay_atm_account'),
        ...
        $this->data['allpay_atm_description'] = nl2br($this->config->get('allpay_atm_description_' . $this->config->get('config_language_id')));
        $this->data['continue'] = $this->url->link('checkout/allpay_atm_success');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/allpay_atm.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/allpay_atm.tpl';
        } else {
            $this->template = 'default/template/payment/allpay_atm.tpl';
        }
        //$this->model_checkout_order->confirm($this->session->data['order_id'], $this->config->get('allpay_atm_order_status_id'), "");
    ...
    public function callback() {
        ...
        $mer_id = $this->config->get('allpay_atm_account');
        $order_finish_statu = $this->config->get('allpay_atm_order_finish_status_id');
