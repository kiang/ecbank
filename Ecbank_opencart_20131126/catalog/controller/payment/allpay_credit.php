...
class ControllerPaymentAllpayCredit extends Controller {
    ...
    protected function index() {
        $this->language->load('payment/allpay_credit');
        ...
        $hash_iv = $this->config->get('allpay_credit_iv_key');
        $hash_key = $this->config->get('allpay_credit_hash_key');
        $merchant_id = $this->config->get('allpay_credit_account');
        $chose_payment = "Credit";
        $trade_date = date("Y/m/d H:i:s");
        $total_amt = intval(round($order_info['total']));
        $return_url = $this->url->link('payment/allpay_credit/callback');
        ...
        $input_array = array(
            "MerchantID" => $this->config->get('allpay_credit_account'),
        ...
        $this->data['allpay_credit_description'] = nl2br($this->config->get('allpay_credit_description_' . $this->config->get('config_language_id')));
        $this->data['continue'] = $this->url->link('checkout/allpay_credit_success');

        if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/allpay_credit.tpl')) {
            $this->template = $this->config->get('config_template') . '/template/payment/allpay_credit.tpl';
        } else {
            $this->template = 'default/template/payment/allpay_credit.tpl';
        }
    ...
    public function callback() {
        ...
        $mer_id = $this->config->get('allpay_credit_account');
        $order_finish_statu = $this->config->get('allpay_credit_order_finish_status_id');
