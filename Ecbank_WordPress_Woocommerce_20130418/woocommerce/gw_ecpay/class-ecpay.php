 ...
 */
add_action('plugins_loaded', 'ecpay_gateway_init', 0);

function ecpay_gateway_init() {
//load_plugin_textdomain( 'ecpay', false, dirname( plugin_basename( __FILE__ ) ) . '/lang/'  ); 
    if (!class_exists('woocommerce_payment_gateway')) {
        return;
    }

    class WC_ecpay extends WC_Payment_Gateway {
...
// Actions
            ...
            add_action('woocommerce_thankyou_ecpay', array(&$this, 'thankyou_page'));  //需與id名稱大小寫相同
...
// Customer Emails
        ...
        function init_form_fields() {  //後台設置欄位
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('啟動 Ecpay 線上刷卡模組', 'woocommerce'),
                    'default' => 'yes'
                ),
                'title' => array(
                    ...
                    'default' => __('Ecpay 線上刷卡', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('客戶訊息', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('', 'woocommerce'),
                    'default' => __(' 綠界 Ecpay - 線上刷卡付款', 'woocommerce')
                ),
                'mer_id' => array(
                    'title' => __('商店代號', 'woocommerce'),
                    'type' => 'text',
                    'description' => __('請填入您Ecpay商店代號', 'woocommerce'),
                    'default' => __('0000', 'woocommerce')
                ),
                'check_code' => array(
                    'title' => __('商家驗證碼', 'woocommerce'),
                    'type' => 'text',
                    'description' => __('請填入您Ecpay商店的商家驗證碼', 'woocommerce'),
        ...
        public function admin_options() {
            ?>
            <h3><?php _e('綠界 Ecpay 線上刷卡模組', 'woocommerce'); ?></h3>
            <p><?php _e('此模組可以讓您使用綠界科技的Ecpay 線上刷卡付款功能', 'woocommerce'); ?></p>
        ...
        /**
         * Get Ecpay Args for passing to Ecbank
         *
         * @access public
         * @param mixed $order
         * @return array
         */
        function get_ecpay_args($order) {
            global $woocommerce;

            $order_id = $order->id;

            if ($this->debug == 'yes') {
                $this->log->add('ecpay', 'Generating payment form for order #' . $order_id . '. Notify URL: ' . $this->notify_url);
            }
            ...
            $ecpay_args = apply_filters('woocommerce_ecpay_args', $ecpay_args);
            return $ecpay_args;
        }
        ...
        function thankyou_page($order_id) {  //接收回傳參數驗證
            ...
            if ($my_spcheck == $_POST['spcheck']) {
                if ($_POST['succ'] == 1) {
                    ...
                    $order->update_status('processing', __('Payment received, awaiting fulfilment', 'ecpay'));
        ...
        /**
         * Generate the ecpay button link (POST method)
         *
         * @access public
         * @param mixed $order_id
         * @return string
         */
        function generate_ecpay_form($order_id) {///***********************************************************
            global $woocommerce;
            $order = new WC_Order($order_id);
            $ecpay_args = $this->get_ecpay_args($order);
            $ecpay_gateway = $this->gateway;
            $ecpay_args_array = array();
            foreach ($ecpay_args as $key => $value) {
                $ecpay_args_array[] = '<input type="hidden" name="' . esc_attr($key) . '" value="' . esc_attr($value) . '" />';
            }
			...
			jQuery("#submit_ecpay_payment_form").click();				
			');

            return '<form id="ecpay" name="ecpay" action=" ' . $ecpay_gateway . ' " method="post" target="_top">' . implode('', $ecpay_args_array) . '
				<input type="submit" class="button-alt" id="submit_ecpay_payment_form" value="' . __('Pay via ecpay', 'ecpay') . '" />
				</form>' . "<script>document.forms['ecpay'].submit();</script>";
        }
        ...
        function receipt_page($order) {
            echo '<p>' . __('感謝您的訂購，接下來將導向向到刷卡頁面，請稍後.', 'ecpay') . '</p>';
            echo $this->generate_ecpay_form($order);
        }
         ...
         * @return void
          ...
          if ( $order->payment_method !== 'ecpay') return;
     ...
     * @return array
     */
    function add_ecpay_gateway($methods) {
        $methods[] = 'WC_ecpay';
        return $methods;
    }

    add_filter('woocommerce_payment_gateways', 'add_ecpay_gateway');
}
?>