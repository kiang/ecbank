 ...
 * Plugin URI: http://www.allpay.com.tw/
 * Description: 綠界Allpay 歐付寶 線上刷卡模組
 * Version: 1.0
...
//load_plugin_textdomain( 'allpay', false, dirname( plugin_basename( __FILE__ ) ) . '/lang/'  ); 
    ...
    class WC_allpay extends WC_Payment_Gateway {
        ...
        public function __construct() {
            $this->id = 'allpay';
            $this->icon = apply_filters('woocommerce_allpay_icon', plugins_url('icon/green_log_40.gif', __FILE__));
...
// Define user set variables
            ...
            $this->gateway = 'https://credit.allpay.com.tw/form_Sc_to5.php';
...
//add_action('init', array(&$this, 'check_allpay_response'));
// Customer Emails
        ...
        function init_form_fields() {  //後台設置欄位
            $this->form_fields = array(
                'enabled' => array(
                    ...
                    'type' => 'checkbox',
                    'label' => __('啟動 Allpay 歐付寶 線上刷卡模組', 'woocommerce'),
                    'default' => 'yes'
                ),
                'title' => array(
                    ...
                    'default' => __('Allpay 歐付寶 線上刷卡', 'woocommerce')
                ),
                'description' => array(
                    'title' => __('客戶訊息', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('', 'woocommerce'),
                    'default' => __(' 綠界 Allpay 歐付寶 線上刷卡付款', 'woocommerce')
                ),
                'mer_id' => array(
                    'title' => __('商店代號', 'woocommerce'),
                    'type' => 'text',
                    'description' => __('請填入您Allpay 歐付寶商店代號', 'woocommerce'),
                    'default' => __('0000', 'woocommerce')
                ),
                'check_code' => array(
                    'title' => __('商家驗證碼', 'woocommerce'),
                    'type' => 'text',
                    'description' => __('請填入您Allpay 歐付寶商店的商家驗證碼', 'woocommerce'),
        ...
        public function admin_options() {
            ?>
            <h3><?php _e('綠界 Allpay 歐付寶 線上刷卡模組', 'woocommerce'); ?></h3>
            <p><?php _e('此模組可以讓您使用綠界科技的Allpay 歐付寶 線上刷卡付款功能', 'woocommerce'); ?></p>
        ...
        function thankyou_page($order_id) {  //接收回傳參數驗證
            ...
            if ($my_spcheck == $_POST['spcheck']) {
                if ($_POST['succ'] == 1) {
                    $result_msg = '交易成功，Allpay 交易單號：' . $_REQUEST['gwsr'] . '，處理日期：' . $_REQUEST['process_date'] .'，時間：'. $_REQUEST['process_time'];   //交易成功
         ...
         * @return void
          ...
          if ( $order->payment_method !== 'allpay') return;
