...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/ecbank_pincode.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'ecbank_pincode_desc';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'ecbank_pincode_account',           'type' => 'text',   'value' => '1111'),
        array('name' => 'ecbank_pincode_checkcode',         'type' => 'text',   'value' => '12742742'),
       	array('name' => 'ecbank_pincode_language',	         'type' => 'select', ' value' => '0'),
        array('name' => 'ecbank_pincode_inv_active',	      'type' => 'select',	'value' => '0'),
        array('name' => 'ecbank_pincode_inv_mer_id',	      'type' => 'text',         'value' => '')
    );
    return;
}
...
class ecbank_pincode
{
    ...
    {
		//print_r($order);exit;
		$payment  = get_payment('ecbank_pincode');
		
		$c_mid		= trim($payment['ecbank_pincode_account']); 
		...
		$notifytype	= "0";
		$c_language	= $payment['ecbank_pincode_language'];
		$c_memo1	= $order['log_id'];
		$c_memo2	= $order['log_id'];
		$key=trim($payment['ecbank_pincode_checkcode']);
		$def_url	= "";

		//開始綠界虛擬帳號取號

                //if (isset($note['action_note']))
		$param = array (
        // ecbank主機
  ...
  // '&desc1='.$param['desc1'].
   ...
   '&ok_url='.$param['ok_url'];
                ...
                //判斷是否使用電子發票
                if($payment['ecbank_pincode_inv_active']=="1"){
                    $nvpStr.="&inv_active=1";
                    $nvpStr.="&inv_mer_id=".$payment['ecbank_pincode_inv_mer_id'];
...
curl_close($ch);
		...
		if(!isset($res['error']) || $res['error'] != '0')
		    ...
		    $def_url.='[全家/萊爾富]超商繳費代碼:(<font color=blue size=+2>'.$res['payno'].'</font>)<br><br>';
			$def_url.='<a href=http://www.ecbank.com.tw/expenses-famiport.htm target=_blank>全家 FamiPort 門市操作步驟</a><br>';
			$def_url.='<a href=http://www.ecbank.com.tw/expenses-life-et.htm target=_blank>萊爾富Life-ET 門市操作步驟</a><br>';
            $def_url.='請記下上列超商繳費代碼,至最近之全家或萊爾富便利商店,操作代碼繳費機台, 於列印出有調碼之繳款單後,至櫃台支付,<br>便可完成繳費,繳費之收據請留存以供備核,繳費之後才算完成購物流程'; 
			$def_url.='<br><br>本線上金流機制採用&lt; <a href=http://www.ecbank.com.tw target=_blank>綠界科技 ECBank 線上支付平台</a> &gt;, 於超商繳費單列印平台可見綠界科技 ECBank 圖示,請安心使用';
    ...
    function respond()
    {
		...
		//驗證碼
		$checkcode = trim($payment['ecbank_pincode_checkcode']);
