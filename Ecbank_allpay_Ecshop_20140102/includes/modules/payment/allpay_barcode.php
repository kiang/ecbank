...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/allpay_barcode.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc'] = 'allpay_barcode_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'allpay_barcode_account', 'type' => 'text', 'value' => '1111'),
        array('name' => 'allpay_barcode_iv', 'type' => 'text', 'value' => 'iv'),
        array('name' => 'allpay_barcode_key', 'type' => 'text', 'value' => 'key')
    );
    return;
}
...
class allpay_barcode {
    ...
    function get_code($order, $payment) {
        $c_mid = trim($payment['allpay_barcode_account']);
        ...
        $def_url = '';
        $key = trim($payment['allpay_barcode_key']);
        $iv = trim($payment['allpay_barcode_iv']);
     ...
     */
    function respond() {
        $payment = get_payment('allpay_barcode');
