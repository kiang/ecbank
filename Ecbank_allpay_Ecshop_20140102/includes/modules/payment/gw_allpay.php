...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/gw_allpay.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'gw_allpay_desc';

    /* 是否支持货到付款 */
    $modules[$i]['is_cod']  = '0';

    /* 作者 */
    $modules[$i]['author']  = 'gw_allpay';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'gw_allpay_account',           'type' => 'text',         'value' => '3'),
        array('name' => 'gw_allpay_checkcode',         'type' => 'text',         'value' => '94499380'),
        array('name' => 'gw_allpay_language',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_allpay_inv_active',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_allpay_inv_mer_id',	      'type' => 'text',         'value' => '')
    );
    return;
}
...
class gw_allpay
{
    ...
    function get_code($order, $payment)
    {
        ...
        $c_mid		= trim($payment['gw_allpay_account']); 
        ...
        $notifytype	= "0";
        $c_language	= $payment['gw_allpay_language'];
        ...
        $def_url  = '<br /><form style="text-align:center;" method=post action="https://credit.allpay.com.tw/form_Sc_to5.php">';
        ...
        //判斷是否使用電子發票
        if($payment['gw_allpay_inv_active']=="1"){
            $def_url .= "<input type='hidden' name='inv_active' value='1'>";
            $def_url .= "<input type='hidden' name='inv_mer_id' value='".$payment['gw_allpay_inv_mer_id']."'>";
            $def_url .= "<input type='hidden' name='inv_amt' value='".$c_orderamount."'>";
    ...
    function respond()
    {
		...
		$c_checkcode	= trim($payment['gw_allpay_checkcode']);
