...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/allpay_topupused.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc'] = 'allpay_topupused_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'allpay_topupused_account', 'type' => 'text', 'value' => '1111'),
        array('name' => 'allpay_topupused_iv', 'type' => 'text', 'value' => 'iv'),
        array('name' => 'allpay_topupused_key', 'type' => 'text', 'value' => 'key')
    );
    return;
}
...
class allpay_topupused {
     ...
     * @return void
     */
    function allpay_topupused() {
        
    }

    function __construct() {
        $this->allpay_topupused();
    }
    ...
    function get_code($order, $payment) {
        ...
        $c_mid = trim($payment['allpay_topupused_account']);
        $log_id = $order['log_id'];
        ...
        $key = trim($payment['allpay_topupused_key']);
        $iv = trim($payment['allpay_topupused_iv']);
     ...
     */
    function respond() {
        $payment = get_payment('allpay_topupused');
