...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/gw_ecpay12.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'gw_ecpay12_desc';

    /* 是否支持货到付款 */
    $modules[$i]['is_cod']  = '0';

    /* 作者 */
    $modules[$i]['author']  = 'gw_ecpay12';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'gw_ecpay12_account',           'type' => 'text',   'value' => '3'),
        array('name' => 'gw_ecpay12_checkcode',         'type' => 'text',   'value' => '94499380'),
        array('name' => 'gw_ecpay12_installment',       'type' => 'text',   'value' => '0'),
        array('name' => 'gw_ecpay12_language',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_ecpay12_inv_active',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_ecpay12_inv_mer_id',	      'type' => 'text',         'value' => '')
    );
    return;
}
...
class gw_ecpay12
{
    ...
    function get_code($order, $payment)
    {
		...
		$c_mid		= trim($payment['gw_ecpay12_account']); 
		...
		$notifytype	= "0";
		$c_language	= $payment['gw_ecpay12_language'];
		...
		if (is_numeric($payment['gw_ecpay12_installment']) && $payment['gw_ecpay12_installment'] > 0){
                    if ($order['pay_fee'] == 0){
		  	$c_orderamount=round($c_orderamount*(1+$payment['gw_ecpay12_installment']));		  	
		  	$pay_button = "".$GLOBALS['_LANG']['pay_button']."".$GLOBALS['_LANG']['gw_ecpay12_stage']."期 ".($payment['gw_ecpay12_installment'] * 100)."% 利率付款 總計金額:".$c_orderamount."元";
			...
			$pay_button = "".$GLOBALS['_LANG']['pay_button']."".$GLOBALS['_LANG']['gw_ecpay12_stage']."期零利率付款";
		}		
		
		$def_url  = '<br /><form style="text-align:center;" method=post action="https://ecpay.com.tw/form_Sc_to5_fn.php">';
		$def_url .= "<input type='hidden' name='client' value='".$c_mid."'>";
		$def_url .= "<input type='hidden' name='act' value='auth'>";
		$def_url .= "<input type='hidden' name='stage' value='".$GLOBALS['_LANG']['gw_ecpay12_stage']."'>";
		...
		$def_url .= "<input type='hidden' name='roturl' value='".$c_returl."'>";
                ...
                if($payment['gw_ecpay12_inv_active']=="1"){

                    $def_url .= "<input type='hidden' name='inv_active' value='1'>";
                    $def_url .= "<input type='hidden' name='inv_mer_id' value='".$payment['gw_ecpay12_inv_mer_id']."'>";
    ...
    function respond()
    {
		...
		$payment  = get_payment('gw_ecpay12');
		//print_r($_REQUEST);
		...
		$c_checkcode	= trim($payment['gw_ecpay12_checkcode']);
		...
		if($my_spcheck!= $_REQUEST['spcheck']  || $_REQUEST['succ']!='1' || $checkAmount!="1"){
			return false;
    			...
    			$note = '綠界交易流水号:'.$c_transnum.' 總金額:'.$c_orderamount;
