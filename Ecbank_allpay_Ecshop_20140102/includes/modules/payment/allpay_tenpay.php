...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/allpay_tenpay.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc'] = 'allpay_tenpay_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'allpay_tenpay_account', 'type' => 'text', 'value' => '1111'),
        array('name' => 'allpay_tenpay_iv', 'type' => 'text', 'value' => 'iv'),
        array('name' => 'allpay_tenpay_key', 'type' => 'text', 'value' => 'key')
    );
    return;
}
...
class allpay_tenpay {
    ...
    function get_code($order, $payment) {
        ...
        $c_mid = trim($payment['allpay_tenpay_account']);
        $log_id = $order['log_id'];
        ...
        $key = trim($payment['allpay_tenpay_key']);
        $iv = trim($payment['allpay_tenpay_iv']);
        ...
        $input_array = array(
            ...
            'TradeDesc' => $desc,
            'ExpireTime' => date("Y/m/d H:i:s" ,mktime(date("H"),date("i"),date("s"),date("m"),date("d")+1,date("Y")))
        );
        ksort($input_array);
     ...
     */
    function respond() {
        $payment = get_payment('allpay_tenpay');
