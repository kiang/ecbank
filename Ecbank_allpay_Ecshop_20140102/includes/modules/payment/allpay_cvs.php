...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/allpay_cvs.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc'] = 'allpay_cvs_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'allpay_cvs_account', 'type' => 'text', 'value' => '1111'),
        array('name' => 'allpay_cvs_iv', 'type' => 'text', 'value' => 'iv'),
        array('name' => 'allpay_cvs_key', 'type' => 'text', 'value' => 'key')
    );
    return;
}
...
class allpay_cvs {
    ...
    function get_code($order, $payment) {
        ...
        $c_mid = trim($payment['allpay_cvs_account']);
        $log_id = $order['log_id'];
        ...
        $key = trim($payment['allpay_cvs_key']);
        $iv = trim($payment['allpay_cvs_iv']);
     ...
     */
    function respond() {
        $payment = get_payment('allpay_cvs');
