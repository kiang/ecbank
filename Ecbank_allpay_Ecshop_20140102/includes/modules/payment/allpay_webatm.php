...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/allpay_webatm.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc'] = 'allpay_webatm_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'allpay_webatm_account', 'type' => 'text', 'value' => '1111'),
        array('name' => 'allpay_webatm_iv', 'type' => 'text', 'value' => 'iv'),
        array('name' => 'allpay_webatm_key', 'type' => 'text', 'value' => 'key')
    );
    return;
}
...
class allpay_webatm {
    ...
    function get_code($order, $payment) {
        ...
        $c_mid = trim($payment['allpay_webatm_account']);
        $log_id = $order['log_id'];
        ...
        $key = trim($payment['allpay_webatm_key']);
        $iv = trim($payment['allpay_webatm_iv']);
     ...
     */
    function respond() {
        $payment = get_payment('allpay_webatm');
