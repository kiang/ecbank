...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/gw_ecpay_union.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'gw_ecpay_union_desc';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'gw_ecpay_union_account',           'type' => 'text',   'value' => '979702'),
        array('name' => 'gw_ecpay_union_checkcode',         'type' => 'text',   'value' => '43157975'),
        //array('name' => 'gw_ecpay_banktype',        'type' => 'text',   'value' => ''),
        array('name' => 'gw_ecpay_union_language',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_ecpay_union_inv_active',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_ecpay_union_inv_mer_id',	      'type' => 'text',         'value' => '')
    );
    return;
}
...
class gw_ecpay_union
{
    ...
    function get_code($order, $payment)
    {
		...
		$c_mid		= trim($payment['gw_ecpay_union_account']); 
		...
		$c_returl	= return_url(basename(__FILE__, '.php'));
                $bk_posturl     = return_url(basename(__FILE__, '.php'));
		$notifytype	= "0";
		$c_language	= $payment['gw_ecpay_union_language'];
		...
		$def_url .= "<input type='hidden' name='時間' value='".$c_ymd."'>";
                $def_url .= "<input type='hidden' name='mallurl' value='".$bk_posturl."'>";
                $def_url .= "<input type='hidden' name='CUPus' value='1'>";
		$def_url .= "<input type='hidden' name='bk_posturl' value='".$c_returl."'>";
                $temp=order_goods($order['order_id']);
                //判斷是否使用電子發票
                if($payment['gw_ecpay_union_inv_active']=="1"){
                    $def_url .= "<input type='hidden' name='inv_active' value='1'>";
                    $def_url .= "<input type='hidden' name='inv_mer_id' value='".$payment['gw_ecpay_union_mer_id']."'>";
