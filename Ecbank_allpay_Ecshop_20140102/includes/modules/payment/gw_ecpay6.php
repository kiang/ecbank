...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/gw_ecpay6.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'gw_ecpay6_desc';

    /* 是否支持货到付款 */
    $modules[$i]['is_cod']  = '0';

    /* 作者 */
    $modules[$i]['author']  = 'gw_ecpay6';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'gw_ecpay6_account',           'type' => 'text',   'value' => '3'),
        array('name' => 'gw_ecpay6_checkcode',         'type' => 'text',   'value' => '94499380'),
        array('name' => 'gw_ecpay6_installment',       'type' => 'text',   'value' => '0'),
        array('name' => 'gw_ecpay6_language',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_ecpay6_inv_active',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_ecpay6_inv_mer_id',	      'type' => 'text',         'value' => '')
    );
    return;
}
...
class gw_ecpay6
{
    ...
    function get_code($order, $payment)
    {
		...
		$c_mid		= trim($payment['gw_ecpay6_account']); 
		...
		$notifytype	= "0";
		$c_language	= $payment['gw_ecpay6_language'];
		...
		if (is_numeric($payment['gw_ecpay6_installment']) && $payment['gw_ecpay6_installment'] > 0){
                        if ($order['pay_fee'] == 0){
                            $c_orderamount=round($c_orderamount*(1+$payment['gw_ecpay6_installment']));		  	
                            $pay_button = "".$GLOBALS['_LANG']['pay_button']."".$GLOBALS['_LANG']['gw_ecpay6_stage']."期 ".($payment['gw_ecpay6_installment'] * 100)."% 利率付款 總計金額:".$c_orderamount."元";
			  ...
			  $pay_button = "".$GLOBALS['_LANG']['pay_button']."".$GLOBALS['_LANG']['gw_ecpay6_stage']."期零利率付款";
		}		
		...
		$def_url .= "<input type='hidden' name='stage' value='".$GLOBALS['_LANG']['gw_ecpay6_stage']."'>";
		...
		$def_url .= "<input type='hidden' name='roturl' value='".$c_returl."'>";
                ...
                //判斷是否使用電子發票
                if($payment['gw_ecpay6_inv_active']=="1"){
                    $def_url .= "<input type='hidden' name='inv_active' value='1'>";
                    $def_url .= "<input type='hidden' name='inv_mer_id' value='".$payment['gw_ecpay6_inv_mer_id']."'>";
    ...
    function respond()
    {
		...
		$c_checkcode	= trim($payment['gw_ecpay6_checkcode']);
