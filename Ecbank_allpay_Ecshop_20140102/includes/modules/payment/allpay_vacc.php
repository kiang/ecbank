...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/allpay_vacc.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc'] = 'allpay_vacc_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'allpay_vacc_account', 'type' => 'text', 'value' => '1111'),
        array('name' => 'allpay_vacc_iv', 'type' => 'text', 'value' => 'iv'),
        array('name' => 'allpay_vacc_key', 'type' => 'text', 'value' => 'key')
    );
    return;
}
...
class allpay_vacc {
    ...
    function get_code($order, $payment) {
        ...
        $c_mid = trim($payment['allpay_vacc_account']);
        $log_id = $order['log_id'];
        ...
        $key = trim($payment['allpay_vacc_key']);
        $iv = trim($payment['allpay_vacc_iv']);
     ...
     */
    function respond() {
        $payment = get_payment('allpay_vacc');
