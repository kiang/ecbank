...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/gw_allpay12.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'gw_allpay12_desc';

    /* 是否支持货到付款 */
    $modules[$i]['is_cod']  = '0';

    /* 作者 */
    $modules[$i]['author']  = 'gw_allpay12';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'gw_allpay12_account',           'type' => 'text',   'value' => '4'),
        array('name' => 'gw_allpay12_checkcode',         'type' => 'text',   'value' => '94499380'),
        array('name' => 'gw_allpay12_installment',       'type' => 'text',   'value' => '0'),
        array('name' => 'gw_allpay12_language',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_allpay12_inv_active',	      'type' => 'select',	'value' => '0'),
        array('name' => 'gw_allpay12_inv_mer_id',	      'type' => 'text',         'value' => '')
    );
    return;
}
...
class gw_allpay12
{
    ...
    function get_code($order, $payment)
    {
		...
		$c_mid		= trim($payment['gw_allpay12_account']); 
		...
		$notifytype	= "0";
		$c_language	= $payment['gw_allpay12_language'];
		...
		if (is_numeric($payment['gw_allpay12_installment']) && $payment['gw_allpay12_installment'] > 0){
                    if ($order['pay_fee'] == 0){
		  	$c_orderamount=round($c_orderamount*(1+$payment['gw_allpay12_installment']));		  	
		  	$pay_button = "".$GLOBALS['_LANG']['pay_button']."".$GLOBALS['_LANG']['gw_allpay12_stage']."期 ".($payment['gw_allpay12_installment'] * 100)."% 利率付款 總計金額:".$c_orderamount."元";
			...
			$pay_button = "".$GLOBALS['_LANG']['pay_button']."".$GLOBALS['_LANG']['gw_allpay12_stage']."期零利率付款";
		}		
		...
		$def_url .= "<input type='hidden' name='stage' value='".$GLOBALS['_LANG']['gw_allpay12_stage']."'>";
		...
		$def_url .= "<input type='hidden' name='roturl' value='".$c_returl."'>";
                ...
                if($payment['gw_allpay12_inv_active']=="1"){

                    $def_url .= "<input type='hidden' name='inv_active' value='1'>";
                    $def_url .= "<input type='hidden' name='inv_mer_id' value='".$payment['gw_allpay12_inv_mer_id']."'>";
    ...
    function respond()
    {
		...
		$payment  = get_payment('gw_allpay12');
		//print_r($_REQUEST);
		...
		$c_checkcode	= trim($payment['gw_allpay12_checkcode']);
