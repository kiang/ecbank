...
// class methods
	...
	if(!$_REQUEST[succ]) {
      ...
      //if ( MODULE_PAYMENT_PAYONLIE_RETUREURL != '') {$reurl = 'returl' ; $returl_var= MODULE_PAYMENT_ECBANK_BARCODE_RETUREURL ; }  
		...
		$c_mid	=	MODULE_PAYMENT_ECBANK_BARCODE_MID;
		$key	=	MODULE_PAYMENT_ECBANK_BARCODE_CHECKCODE;
		$price=intval($_REQUEST[amt]);
		
		$get_vacc ='https://ecbank.com.tw/gateway.php?';
		$get_vacc.='mer_id='.$c_mid.'&';
		$get_vacc.='payment_type=barcode&';
		$get_vacc.='prd_desc='.$ordnum.'&';
		$get_vacc.='enc_key='.$key.'&';
		$get_vacc.='od_sob='.$ordnum.'&';
		$get_vacc.='amt='.$price.'&';
		$get_vacc.='expire_day=3&';
		$get_vacc.='ok_url='.rawurlencode($roturl);
		//echo $c_returl.'<hr>';
		echo $get_vacc."<hr>";
		//exit;
		$vacc_code= file_get_contents($get_vacc);

		parse_str($vacc_code, $res);
	  //  echo $res['error'];
	  //  exit;
		if($res['error']==0) {
    ...
    $checkcode=MODULE_PAYMENT_ECBANK_BARCODE_CHECKCODE;
    ...
    function get_error() {
	  	switch ($_GET['error']) {
		  		case 1 :
		  				$error_message = MODULE_PAYMENT_ECBANK_BARCODE_TEXT_ERROR_1 ;
		  				break ;
		 		 case 2 :
		  				$error_message = MODULE_PAYMENT_ECBANK_BARCODE_TEXT_ERROR_2 ;
		  				break ;
		  		case 3 :
		  				$error_message = MODULE_PAYMENT_ECBANK_BARCODE_TEXT_ERROR_3 ;
		 					 break ;
	  	}
      $error = array('title' => MODULE_PAYMENT_ECBANK_BARCODE_TEXT_ERROR,
                     'error' => $error_message );
      return $error;
    }
*/
