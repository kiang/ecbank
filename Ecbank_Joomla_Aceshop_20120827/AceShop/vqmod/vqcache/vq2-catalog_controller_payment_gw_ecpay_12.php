...
class ControllerPaymentGwEcpay12 extends Controller {
	protected function index() {
		...
		$this->language->load('payment/gw_ecpay_12');
		
		$od_sob = $this->session->data['order_id']; //取得訂單編號
		$order_info = $this->model_checkout_order->getOrder($od_sob);//取得訂單資訊
		$client = $this->config->get('gw_ecpay_12_account'); //取得ECPAY商店代號
		$SMethod = $this->config->get('gw_ecpay_12_stagemethod');//取得分期成本計算方法
		$cfgValue = $this->config->get('gw_ecpay_12_cfg_value');//取得分期利率
		...
		$roturl = HTTPS_SERVER . 'index.php?option=com_aceshop&route=payment/gw_ecpay_12/callback'; //付款完畢觸發網址
		$bk_posturl = HTTPS_SERVER . 'index.php?option=com_aceshop&route=payment/gw_ecpay_12/bk_callback';
                $i_invoice = $this->config->get('gw_ecpay_12_i_invoice');//電子發票
                $imer_id = $this->config->get('gw_ecpay_12_imer_id');
                $delay = $this->config->get('gw_ecpay_12_delay');//電子發票延遲天數
		...
		//定義表單
		$this->data['def_url']  = "<form name='form1' method='post' action='https://ecpay.com.tw/form_Sc_to5_fn.php'>";
		...
		$this->data['def_url'] .= "<input type='hidden' name='stage' value='12'>";
                if($i_invoice == 'yes'){
		...
		$this->data['gw_ecpay_12_description'] = nl2br($this->config->get('gw_ecpay_12_description_' . $this->config->get('config_language_id')));
		
		//$this->data['continue'] = HTTPS_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_12_respond';//完成結帳網址
		
		//載入樣板
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gw_ecpay_12.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/gw_ecpay_12.tpl';
		} else {
			$this->template = 'default/template/payment/gw_ecpay_12.tpl';
		}
	...
	public function confirm(){
		...
		if(!isset($this->session->data['doubleclick']) && isset($this->session->data['order_id'])){
			...
			$od_sob = $this->session->data['order_id'];//取得訂單編號
			$order_info = $this->model_checkout_order->getOrder($od_sob);//取得訂單資訊
			$orderStatus = $this->config->get('gw_ecpay_12_order_status_id');//取得預設訂單狀態
			$SMethod = $this->config->get('gw_ecpay_12_stagemethod');//取得分期成本計算方法
			$cfgValue = $this->config->get('gw_ecpay_12_cfg_value');//取得分期利率
	...
	public function callback() {
		if (
			...
			$c_checkcode = $this->config->get('gw_ecpay_12_checkcode'); //綠界 ECPAY 檢查碼
			$TOkSi = $_POST['process_time'] + $_POST['gwsr'] + $_POST['amount'];
			$my_spcheck = $this->gwSpcheck($c_checkcode, $TOkSi);
			$ecbank_gateway = HTTP_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_12_respond';
