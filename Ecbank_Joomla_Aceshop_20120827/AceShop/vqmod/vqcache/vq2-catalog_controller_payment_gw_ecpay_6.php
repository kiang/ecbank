...
class ControllerPaymentGwEcpay6 extends Controller {
	protected function index() {
		...
		$this->language->load('payment/gw_ecpay_6');
		
		$od_sob = $this->session->data['order_id']; //取得訂單編號
		$order_info = $this->model_checkout_order->getOrder($od_sob);//取得訂單資訊
		$client = $this->config->get('gw_ecpay_6_account'); //取得ECPAY商店代號
		$SMethod = $this->config->get('gw_ecpay_6_stagemethod');//取得分期成本計算方法
		$cfgValue = $this->config->get('gw_ecpay_6_cfg_value');//取得分期利率
		...
		$roturl = HTTPS_SERVER . 'index.php?option=com_aceshop&route=payment/gw_ecpay_6/callback'; //付款完畢觸發網址
		$bk_posturl = HTTPS_SERVER . 'index.php?option=com_aceshop&route=payment/gw_ecpay_6/bk_callback';
                $i_invoice = $this->config->get('gw_ecpay_6_i_invoice');//電子發票
                $imer_id = $this->config->get('gw_ecpay_6_imer_id');
                $delay = $this->config->get('gw_ecpay_6_delay');//電子發票延遲天數
		...
		$this->data['def_url'] .= "<input type='hidden' name='stage' value='6'>";
                if($i_invoice == 'yes'){
		...
		$this->data['gw_ecpay_6_description'] = nl2br($this->config->get('gw_ecpay_6_description_' . $this->config->get('config_language_id')));
		
		//$this->data['continue'] = HTTPS_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_6_respond';//完成結帳網址
		
		//載入樣板
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gw_ecpay_6.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/gw_ecpay_6.tpl';
		} else {
			$this->template = 'default/template/payment/gw_ecpay_6.tpl';
		}
	...
	public function confirm(){
		...
		if(!isset($this->session->data['doubleclick']) && isset($this->session->data['order_id'])){
			...
			$od_sob = $this->session->data['order_id'];//取得訂單編號
			$order_info = $this->model_checkout_order->getOrder($od_sob);//取得訂單資訊
			$orderStatus = $this->config->get('gw_ecpay_6_order_status_id');//取得預設訂單狀態
			$SMethod = $this->config->get('gw_ecpay_6_stagemethod');//取得分期成本計算方法
			$cfgValue = $this->config->get('gw_ecpay_6_cfg_value');//取得分期利率
	...
	public function callback() {
		if (
			...
			$c_checkcode = $this->config->get('gw_ecpay_6_checkcode'); //綠界 ECPAY 檢查碼
			$TOkSi = $_POST['process_time'] + $_POST['gwsr'] + $_POST['amount'];
			$my_spcheck = $this->gwSpcheck($c_checkcode, $TOkSi);
			$ecbank_gateway = HTTP_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_6_respond';
