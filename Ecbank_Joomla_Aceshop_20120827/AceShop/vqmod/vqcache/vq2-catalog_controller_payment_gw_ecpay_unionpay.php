...
class ControllerPaymentGwEcpayUnionpay extends Controller {
	protected function index() {
		$this->language->load('payment/gw_ecpay_unionpay');
		...
		$client = $this->config->get('gw_ecpay_unionpay_account'); // 您的ECPAY商店代號
		//$amount = (int)$order_info['total']; // 交易金額
		$od_sob = $this->session->data['order_id']; //賣家自訂交易編號
		$roturl = HTTPS_SERVER . 'index.php?route=payment/gw_ecpay_unionpay/callback'; // 回傳網址
		$bk_posturl = HTTPS_SERVER . 'index.php?route=payment/gw_ecpay_unionpay/bk_callback';
		$mallurl = HTTP_SERVER . 'index.php?route=checkout/gw_ecpay_unionpay_respond/unionpay';
		$SMethod = $this->config->get('gw_ecpay_unionpay_stagemethod');//取得分期成本計算方法
		$cfgValue = $this->config->get('gw_ecpay_unionpay_cfg_value');//取得分期利率
		...
		$amount = $this->calculationPee($SMethod,$cfgValue,$order_info['total']);//計算分期刷卡消費金額
                $i_invoice = $this->config->get('gw_ecpay_unionpay_i_invoice');//電子發票
                $imer_id = $this->config->get('gw_ecpay_unionpay_imer_id');
                $delay = $this->config->get('gw_ecpay_unionpay_delay');//電子發票延遲天數
		...
		$this->data['def_url']  = "<form name='form1' method='post' action='https://ecpay.com.tw/form_Sc_to5.php'>";
		...
		$this->data['def_url'] .= "<input type='hidden' name='mallurl' value='".$mallurl."'>";
                $this->data['def_url'] .= "<input type='hidden' name='CUPus' value='1' />";
                if($i_invoice == 'yes'){
		...
		$this->data['gw_ecpay_unionpay_description'] = nl2br($this->config->get('gw_ecpay_unionpay_description_' . $this->config->get('config_language_id')));
		
		$this->data['continue'] = HTTPS_SERVER . 'index.php?route=checkout/gw_ecpay_unionpay_respond';
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gw_ecpay_unionpay.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/gw_ecpay_unionpay.tpl';
		} else {
			$this->template = 'default/template/payment/gw_ecpay_unionpay.tpl';
		}
		$this->render(); 
	}
	public function confirm() {
		...
		if(!isset($this->session->data['doubleclick'])){
			...
			$orderStatus = $this->config->get('gw_ecpay_unionpay_order_status_id'); //取得預設訂單狀態
	...
	public function callback() {
		if (
			...
			$c_checkcode = $this->config->get('gw_ecpay_unionpay_checkcode'); //綠界 ECPAY 檢查碼
			$TOkSi = $_POST['process_time'] + $_POST['gwsr'] + $_POST['amount'];
			$my_spcheck = $this->gwSpcheck($c_checkcode, $TOkSi);
			$ecbank_gateway = HTTP_SERVER . 'index.php?route=checkout/gw_ecpay_unionpay_respond';
