...
class ControllerPaymentGwEcpay3 extends Controller {
	protected function index() {
		...
		$this->language->load('payment/gw_ecpay_3');
		
		$od_sob = $this->session->data['order_id']; //取得訂單編號
		$order_info = $this->model_checkout_order->getOrder($od_sob);//取得訂單資訊
		$client = $this->config->get('gw_ecpay_3_account'); //取得ECPAY商店代號
		$SMethod = $this->config->get('gw_ecpay_3_stagemethod');//取得分期成本計算方法
		$cfgValue = $this->config->get('gw_ecpay_3_cfg_value');//取得分期利率
		...
		$roturl = HTTPS_SERVER . 'index.php?option=com_aceshop&route=payment/gw_ecpay_3/callback'; //付款完畢觸發網址
		$bk_posturl = HTTPS_SERVER . 'index.php?option=com_aceshop&route=payment/gw_ecpay_3/bk_callback';
                $i_invoice = $this->config->get('gw_ecpay_3_i_invoice');//電子發票
                $imer_id = $this->config->get('gw_ecpay_3_imer_id');
                $delay = $this->config->get('gw_ecpay_3_delay');//電子發票延遲天數
		...
		$this->data['def_url'] .= "<input type='hidden' name='stage' value='3'>";
                if($i_invoice == 'yes'){
		...
		$this->data['gw_ecpay_3_description'] = nl2br($this->config->get('gw_ecpay_3_description_' . $this->config->get('config_language_id')));
		
		//$this->data['continue'] = HTTPS_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_3_respond';//完成結帳網址
		
		//載入樣板
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gw_ecpay_3.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/gw_ecpay_3.tpl';
		} else {
			$this->template = 'default/template/payment/gw_ecpay_3.tpl';
		}
	...
	public function callback() {
		if (
			...
			$c_checkcode = $this->config->get('gw_ecpay_3_checkcode'); //綠界 ECPAY 檢查碼
			$TOkSi = $_POST['process_time'] + $_POST['gwsr'] + $_POST['amount'];
			$my_spcheck = $this->gwSpcheck($c_checkcode, $TOkSi);
			$ecbank_gateway = HTTP_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_3_respond';
