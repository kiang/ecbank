...
class ControllerPaymentGwEcpay extends Controller {
	protected function index() {
		$this->language->load('payment/gw_ecpay');
		...
		$client = $this->config->get('gw_ecpay_account'); // 您的ECPAY商店代號
		$SMethod = $this->config->get('gw_ecpay_stagemethod');//取得分期成本計算方法
		$cfgValue = $this->config->get('gw_ecpay_cfg_value');//取得分期利率
		...
		$roturl = HTTPS_SERVER . 'index.php?route=payment/gw_ecpay/callback'; // 回傳網址
		$bk_posturl = HTTPS_SERVER . 'index.php?route=payment/gw_ecpay/bk_callback';
		$mallurl = HTTP_SERVER . 'index.php?route=checkout/gw_ecpay_respond/unionpay';
                $i_invoice = $this->config->get('gw_ecpay_i_invoice');//電子發票
                $imer_id = $this->config->get('gw_ecpay_imer_id');
                $delay = $this->config->get('gw_ecpay_delay');//電子發票延遲天數
		...
		$this->data['gw_ecpay_description'] = nl2br($this->config->get('gw_ecpay_description_' . $this->config->get('config_language_id')));
		
		$this->data['continue'] = HTTPS_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_respond';
		
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/gw_ecpay.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/gw_ecpay.tpl';
		} else {
			$this->template = 'default/template/payment/gw_ecpay.tpl';
		}
		$this->render(); 
	}
	public function confirm() {
		...
		if(!isset($this->session->data['doubleclick'])){
			...
			$order_info = $this->model_checkout_order->getOrder($od_sob); //取得訂單資訊
                        $orderStatus = $this->config->get('gw_ecpay_order_status_id'); //取得預設訂單狀態
                        $SMethod = $this->config->get('gw_ecpay_stagemethod');//取得分期成本計算方法
                        $cfgValue = $this->config->get('gw_ecpay_cfg_value');//取得分期利率
	...
	public function callback() {
		if (
			...
			$c_checkcode = $this->config->get('gw_ecpay_checkcode'); //綠界 ECPAY 檢查碼
			$TOkSi = $_POST['process_time'] + $_POST['gwsr'] + $_POST['amount'];
			$my_spcheck = $this->gwSpcheck($c_checkcode, $TOkSi);
			$ecbank_gateway = HTTP_SERVER . 'index.php?option=com_aceshop&route=checkout/gw_ecpay_respond';
