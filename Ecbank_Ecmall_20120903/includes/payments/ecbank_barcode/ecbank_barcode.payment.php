...
class Ecbank_barcodePayment extends BasePayment
{
    
    var $_gateway   =   'https://ecbank.com.tw/gateway.php';
    var $_code      =   'ecbank_barcode';
    ...
    function get_payform($order_info)
    {
        ...
        /* 平台提供者,綠界科技商店代號 */
        $mer_id = $this->_config['ecbank_barcode_account'];
		
		// 交易加密私鑰
	$checkcode = $this->_config['ecbank_barcode_key'];
								   

        $seller = $this->_config['ecbank_barcode_account'];
	...
	//$roturl		=$this->_create_notify_url($order_info['order_id']);
        ...
        //判斷是否使用電子發票
        $i_invoice  = $this->_config['ecbank_barcode_i_invoice'];
        //電子發票商店代號
        $imer_id = $this->_config['ecbank_barcode_imer_id'];
...
document.forms.ecpay.submit();
    ...
    $lineurl = 'https://ecbank.com.tw/order/barcode_print.php?mer_id='.$notify[mer_id].'&tsr='.$notify[tsr].'';	
    
	$ookk=	'<script type="text/javascript">window.onload = function () {
 			var htmlECBank = "</b><font color=red>尚未付款<br>您已經完成訂單作業,此訂單的超商繳費代碼:<font color=blue>'.$notify[od_sob].'</font><br>請列印超商條碼帳單至超商繳費 [<a href='.$lineurl.' target=_blank>點此列印</a>] ";
