...
class Ecbank_ibonPayment extends BasePayment
{
    
    var $_gateway   =   'https://ecbank.com.tw/gateway.php';
    var $_code      =   'ecbank_ibon';
    ...
    function get_payform($order_info)
    {
        ...
        /* 平台提供者,綠界科技商店代號 */
        $mer_id = $this->_config['ecbank_ibon_account'];
		
		// 交易加密私鑰
		$checkcode = $this->_config['ecbank_ibon_key'];
								   

        $seller = $this->_config['ecbank_ibon_account'];
        ...
        //判斷是否使用電子發票
        $i_invoice  = $this->_config['ecbank_ibon_i_invoice'];
        //電子發票商店代號
        $imer_id = $this->_config['ecbank_ibon_imer_id'];
        /*買家email*/
...
document.forms.ecpay.submit();
	...
	$ookk=	'<script type="text/javascript">window.onload = function () {
 				var htmlECBank = "<font color=red>尚未付款<br>已完成訂單作業,此訂單的超商繳費代碼<font color=blue>'.$notify[payno].'</font><br>抄寫下號碼後,您可就近到 [統一超商] 代碼付款機台操作列印付款單據,持單據至櫃台付款能算完成付款作業<hr><a target=_blank href=http://www.ecbank.com.tw/expenses-1.htm>超商代碼付款流程請看此</a>";
