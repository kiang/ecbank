...
class EcpayPayment extends BasePayment
{
    ...
    function get_payform($order_info)
    {
        ...
        /* 平台提供者,綠界科技商店代號 */
        $chnid = $this->_config['ecpay_account'];

        $seller = $this->_config['ecpay_account'];
        ...
        //判斷是否使用電子發票
        $i_invoice  = $this->_config['ecpay_i_invoice'];
        //電子發票商店代號
        $imer_id = $this->_config['ecpay_imer_id'];
        /*買家email*/
...
回
    ...
    function verify_notify($order_info, $strict = false)
    {
	   ...
	   if (empty($order_info))
		...
		if($notify['spcheck']) {
			$gwcheck=gwSpcheck($notify['process_time'],$notify['gwsr'],$notify['amount'],$notify['spcheck'],$this->_config['ecpay_key']);
