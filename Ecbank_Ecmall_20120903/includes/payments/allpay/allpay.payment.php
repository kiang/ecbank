...
class AllpayPayment extends BasePayment
{
    
    var $_gateway   =   'https://credit.allpay.com.tw/form_Sc_to5.php';
    var $_code      =   'allpay';
    ...
    function get_payform($order_info)
    {
        ...
        /* 平台提供者,綠界科技商店代號 */
        $chnid = $this->_config['allpay_account'];

        $seller = $this->_config['allpay_account'];
        ...
        $i_invoice  = $this->_config['ecbank_allpay_i_invoice'];
        //電子發票商店代號
        $imer_id = $this->_config['ecbank_allpay_imer_id'];
        /*買家email*/
...
回
    ...
    function verify_notify($order_info, $strict = false)
    {
	   ...
	   if (empty($order_info))
		...
		if($notify['spcheck']) {
			$gwcheck=gwSpcheck($notify['process_time'],$notify['gwsr'],$notify['amount'],$notify['spcheck'],$this->_config['allpay_key']);
