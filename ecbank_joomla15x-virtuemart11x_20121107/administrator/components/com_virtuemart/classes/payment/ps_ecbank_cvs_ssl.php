...
class ps_ecbank_cvs_ssl {

    var $classname = "ps_ecbank_cvs_ssl";
    var $payment_code = "綠界科技 ECBank";
	//define("ECBANK_CVS_MID",'');
	//error_reporting(0);
...
if($socket) {
 ...
 $req = "GET /gateway.php?$nvpStr HTTP/1.1".chr(13).chr(10).
   		"Host: {$param['ecbank_host']}".chr(13).chr(10).
   		"Content-type: application/x-www-form-urlencoded".chr(13).chr(10).
  		"Connection: Close".chr(13).chr(10).chr(13).chr(10);
 fwrite($socket,$req);

 $http_status = fgets($socket,1024);
 if(eregi("^HTTP/1.. 200 OK", $http_status)) {
  $isheader = 1;
  while(!feof($socket)){
   $line = fgets($socket,2048);
   if($line == chr(10) || $line == chr(13).chr(10)) {
    $strAuth = fgets($socket,2048);
    break;
   }
  }
 }
